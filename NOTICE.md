# Third-Party Notices & Acknowledgments

PriestyAI is built with and inspired by the following incredible open-source projects. All respective copyrights, trademarks, and licenses belong to their respective owners.

### Core Frameworks & AI Runtimes
*   **discord.py:** [MIT License](https://github.com/Rapptz/discord.py/blob/master/LICENSE)
*   **Google Generative AI SDK for Python (`google-genai`):** [Apache-2.0 License](https://github.com/googleapis/python-genai/blob/main/LICENSE)
*   **SearXNG Metasearch Engine:** [AGPL-3.0 License](https://github.com/searxng/searxng/blob/master/LICENSE)
*   **Playwright for Python:** [Apache-2.0 License](https://github.com/microsoft/playwright-python/blob/main/LICENSE)
*   **edge-tts:** [GPL-3.0 License](https://github.com/rany2/edge-tts/blob/master/LICENSE)

### Networking, Async I/O & HTTP
*   **aiohttp:** [Apache-2.0 License](https://github.com/aio-libs/aiohttp/blob/master/LICENSE.txt)
*   **httpx:** [BSD 3-Clause License](https://github.com/encode/httpx/blob/master/LICENSE.md)

### Data Validation, Parsing & Extraction
*   **pydantic:** [MIT License](https://github.com/pydantic/pydantic/blob/main/LICENSE)
*   **beautifulsoup4:** [MIT License](https://www.crummy.com/software/BeautifulSoup/)
*   **trafilatura:** [GPL-3.0 License](https://github.com/adbar/trafilatura/blob/master/LICENSE)
*   **dateparser:** [BSD 3-Clause License](https://github.com/scrapinghub/dateparser/blob/master/LICENSE)
*   **pycountry:** [LGPL-2.1 License](https://github.com/pycountry/pycountry/blob/main/LICENSE.txt)
*   **numpy:** [BSD 3-Clause License](https://github.com/numpy/numpy/blob/main/LICENSE.txt)

### Document & Media Processing
*   **pdfplumber:** [MIT License](https://github.com/jsvine/pdfplumber/blob/master/LICENSE.txt)
*   **pypdf:** [BSD 3-Clause License](https://github.com/py-pdf/pypdf/blob/main/LICENSE)
*   **python-docx:** [MIT License](https://github.com/python-openxml/python-docx/blob/master/LICENSE)
*   **python-pptx:** [MIT License](https://github.com/scanny/python-pptx/blob/master/LICENSE)
*   **openpyxl:** [MIT License](https://foss.heptapod.net/openpyxl/openpyxl/-/blob/branch/default/LICENCE.rst)
*   **Pillow:** [HPND License](https://github.com/python-pillow/Pillow/blob/main/LICENSE)

### Security, Auth & Environment
*   **cryptography:** [Dual Apache-2.0 / BSD 3-Clause License](https://github.com/pyca/cryptography/blob/main/LICENSE)
*   **PyJWT:** [MIT License](https://github.com/jpadilla/pyjwt/blob/master/LICENSE)
*   **python-dotenv:** [BSD 3-Clause License](https://github.com/theskumar/python-dotenv/blob/main/LICENSE)

### Canvas Web Playground
*   **Monaco Editor:** [MIT License](https://github.com/microsoft/monaco-editor/blob/main/LICENSE)
*   **xterm.js:** [MIT License](https://github.com/xtermjs/xterm.js/blob/master/LICENSE)
*   **Marked.js:** [MIT License](https://github.com/markedjs/marked/blob/master/LICENSE)
*   **Highlight.js:** [BSD 3-Clause License](https://github.com/highlightjs/highlight.js/blob/main/LICENSE)
*   **JSZip:** [MIT License](https://github.com/Stuk/jszip/blob/main/LICENSE.markdown)

### UI & Icon Assets
*   **GitHub Octicons:** [MIT License](https://github.com/primer/octicons/blob/main/LICENSE)
*   **Visual Studio Code Codicons:** [Creative Commons Attribution 4.0 International Public License](https://github.com/microsoft/vscode-codicons/blob/main/LICENSE)
*   **Seti UI / File Icons:** [MIT License](https://github.com/jesseweed/seti-ui/blob/master/LICENSE.md) — Used for file extension categorization.


---

### Trademark Notice

All product names, logos, brands, and trademarks displayed or referenced in this repository (including, but not limited to, Discord, GitHub, Python, Docker, Rust, Go, JavaScript, TypeScript, and Microsoft) are the property of their respective owners. 

Their inclusion in this repository and associated applications is solely for nominative identification, interoperability, and file-type representation purposes and does not imply any affiliation, sponsorship, or endorsement.