...
def add_hyperlink(paragraph, url: str, text: str, color: str = "4183C4", underline: bool = True):
    try:
...
def convert_markdown_to_docx(doc_instance, markdown_text: str, double_spaced: bool = False):
...
def generate_comparison_chart(fact_bank: List[Dict[str, Any]], filename: str = "chart.png") -> bool:
    ...
    for item in fact_bank:
        ...
        fact = item.get("fact", "")
        if not entity or not fact or entity.lower() == "none" or entity.lower() in ("various", "all"):
            continue
    ...
    if len(entity_metrics) < 2:
        logger.info("Skipping chart generation: Insufficient comparative datasets discovered.")
        return False
    ...
    unit_label = f"({detected_unit.upper()})" if detected_unit and detected_unit != "$" else ""
    draw.text((30, 20), f"Web Research Comparative Data Matrix {unit_label}".strip(), fill="#ffffff")
...
class DeepResearchSession(BaseAgentSession):
    ...
    async def execute_tick(self, bot):
        try:
            ...
            await self.channel.send(f"🛑 **Research Interrupted:** An unexpected crash occurred during synthesis mapping: `{e}`")
    ...
    async def run_research(self, bot):
        ...
        candidate_slice_limit = 15 if self.depth == "brief" else 45 if self.depth == "standard" else 75 if self.depth == "exhaustive" else 100
        
        selected_candidates = await self.filter_relevance(bot, candidates[:candidate_slice_limit], limit=max_links)
        ...
        self.current_thought = "Checking database for conflicting measurements and data discrepancies..."
        await self.update_progress_msg(bot, active_idx=3, custom_header="🔍 *Identifying discrepancies and resolving conflicts inside fact databases...*")
        ...
        if self.depth != "brief" and len(self.sources) > 0:
            ...
            if gap_queries:
                ...
                await self.update_progress_msg(bot, active_idx=3, custom_header=f"📡 *Recursive pass: Scraping `{len(gap_queries)}` secondary targets...*")
        ...
        if self.format_type == "mla":
            self.current_thought = "Executing isolated scholarly bibliography formatting pass..."
            await self.update_progress_msg(bot, active_idx=3, custom_header="📚 *Compiling structured MLA Works Cited index...*")
        ...
        await self.update_progress_msg(bot, active_idx=4, custom_header="✍️ *Synthesizing collected fact indexes and generating formatted document...*")
        ...
        if self.format_type in ("docx", "mla"):
            ...
            try:
                ...
                if chart_generated and self.format_type != "mla":
                    ...
                    doc.add_paragraph("Figure 1.0: Compiled specifications visual matrix.")
                
                convert_markdown_to_docx(doc, report_markdown, double_spaced=(self.format_type == "mla"))
                ...
                mention_prefix = f"🛎️ <@{self.user_id}>, \n" if self.depth == "extreme" else ""
    ...
    async def generate_custom_outline(self, bot):
        max_chapters = 2 if self.depth == "brief" else 3 if self.depth == "standard" else 4 if self.depth == "exhaustive" else 5
        
        layout_flavor_guide = "academic paper essay structure" if self.format_type == "mla" else f"corporate `{self.flavor.upper()}` structure"
        ...
        try:
            ...
            out_clean = re.sub(r'^```json\s*|\s*```$', '', (response.text or "").strip(), flags=re.IGNORECASE).strip()
            parsed = json.loads(out_clean)
            if isinstance(parsed, dict) and len(parsed) > 0:
                self.clean_title = parsed.get("title", "Analytical Research Dossier").strip()
                self.chapters_outline = parsed.get("outline", self.chapters_outline)
        except Exception as outline_err:
            logger.warning(f"Dynamic outline generation failed (applying fallback placeholders): {outline_err}")
    ...
    async def reconcile_contradictions(self, bot):
        ...
        try:
            ...
            if isinstance(purge_indices, list) and purge_indices:
                logger.info(f"Reconciliation engine identified contradictions. Removing indices: {purge_indices}")
                self.fact_bank = [item for i, item in enumerate(self.fact_bank) if i not in purge_indices]
        except Exception as e:
            logger.warning(f"Contradiction check failed (skipping resolution pass): {e}")
    ...
    async def compile_works_cited_mla(self, bot):
        ...
        try:
            ...
            if isinstance(citations_list, list):
                ...
                logger.info(f"Scholarly citations compiled successfully. Works Cited count: {len(self.works_cited_list)}")
        except Exception as e:
            logger.error(f"Scholarly MLA works cited compilation pass failed: {e}")
            self.works_cited_list = [f"\"Title: {s['title']}.\" Web URL: [Link]({s['url']})" for s in self.sources]
    ...
    async def compile_report_sequential(self, bot) -> str:
        ...
        def filter_relevant_facts(chapter_goal: str) -> str:
            ...
            def get_clean_publisher(source_idx: int) -> str:
                if 0 < source_idx <= len(self.sources):
                    ...
                    parts = domain.split(".")
                    name = parts[0] if parts[0] not in ("blog", "docs", "news", "journals", "pmc") else parts[1]
                    return name.title()
            ...
            for f in self.fact_bank:
                ...
                keywords = re.findall(r'\b\w{4,}\b', chapter_goal.lower() + " " + ent.lower() + " " + attr.lower())
                if any(kw in fact.lower() for kw in keywords if kw not in ("about", "with", "this", "from", "that")):
        ...
        for ch_key, ch_spec in self.chapters_outline.items():
            ...
            self.partial_drafts[ch_title] = f"*Status: Actively drafting chapter content in real-time...*\n\n**Objective:** {ch_obj}"
            ...
            try:
                response = await bot.chat_handler.client.aio.models.generate_content(
                    model=bot.chat_handler.premium_model, contents=ch_prompt,
