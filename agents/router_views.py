...
class ResearchPanelTabView(discord.ui.LayoutView):
    ...
    def rebuild_layout(self) -> None:
        ...
        if self.active_tab == "thinking":
            container.add_item(discord.ui.TextDisplay(content="### 🧠 Deep Research - Active Thinking & Reasoning"))
        ...
        thinking_btn = discord.ui.Button(
            label="Show Thinking", 
            style=discord.ButtonStyle.primary if self.active_tab == "thinking" else discord.ButtonStyle.secondary,
            emoji="🧠"
        )
...
class ViewResearchButton(discord.ui.Button):
    def __init__(self, session_instance):
        super().__init__(style=discord.ButtonStyle.secondary, label="View Research Details", emoji="🧠", custom_id=f"deep_res_btn_{session_instance.thread_id}")
        self.session = session_instance
...
class AgentLaunchModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await interaction.message.edit(view=None, content="✅ *This analysis session has been configured and initialized.*")
...
class DeepResearchLaunchModal(discord.ui.Modal):
    def __init__(self, bot_instance, prompt: str, user: discord.User):
        ...
        self.flavor_select = discord.ui.RadioGroup(options=[
            discord.RadioGroupOption(label="Executive Summary", value="executive", description="High-level value mapping, cost audits, and price guides.", default=True),
            discord.RadioGroupOption(label="Technical Deep Dive", value="technical", description="Dense hardware specs, network latencies, and system audits."),
            discord.RadioGroupOption(label="Comparative Matrix", value="matrix", description="Side-by-side matrices and clear use-case recommendations.")
        ])
        self.add_item(discord.ui.Label(
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await interaction.message.edit(view=None, content="✅ *This research session has been configured and initialized.*")
        except Exception as e:
            logger.warning(f"Could not disable parent setup message: {e}")
        ...
        format_type = self.format_select.value if self.format_select.value else "markdown"
        ...
        guidelines = self.guidelines_input.value.strip() if self.guidelines_input.value else ""
...
class AgentRouterView(discord.ui.LayoutView):
    def __init__(self, bot_instance, user: discord.User, prompt: str, initial_agent: str, initial_plan: str):
        super().__init__(timeout=600.0)
        self.bot = bot_instance
        self.user = user
        self.prompt = prompt
    ...
    def rebuild_layout(self) -> None:
        ...
        title_label = "Deep Research" if self.agent_type == "deep_research" else "Discord Analysis"
        
        container.add_item(discord.ui.TextDisplay(content="*Review my proposed plan below. Click Configure to proceed, or Change Type to manually toggle the operational mode.*"))
        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small, visible=True))
        
        container.add_item(discord.ui.TextDisplay(content=f"### {title_icon} Agent Session - {title_label}"))
        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small, visible=True))
        ...
        config_btn = discord.ui.Button(label="Configure and Start Agent", style=discord.ButtonStyle.success, emoji="🚀")
        config_btn.callback = self.on_configure_click
        action_row.add_item(config_btn)
        
        change_btn = discord.ui.Button(label="Change Type", style=discord.ButtonStyle.secondary, emoji="🔄")
    ...
    async def on_configure_click(self, interaction: discord.Interaction):
        if interaction.user.id != self.user.id:
            await interaction.response.send_message("❌ This setup panel belongs to another user.", ephemeral=True)
            return
        ...
        if self.agent_type == "deep_research":
            ...
            all_contexts = await memory.fetch_all_contexts_for_user(self.bot, self.bot.brain_server_id, self.user.id)
            user_contexts = [c for c in all_contexts if c.get("type") == "User Profile Snapshot"]
            message_contexts = [c for c in all_contexts if c.get("type") == "Message Transcript Snippet"]
    ...
    async def on_change_click(self, interaction: discord.Interaction):
        ...
        container = discord.ui.Container()
        container.add_item(discord.ui.TextDisplay(content="### 🔄 Switch Agent Session Mode\nSelect which operational protocol you wish to apply to this prompt:"))
        container.add_item(discord.ui.Separator(spacing=discord.SeparatorSpacing.small, visible=True))
