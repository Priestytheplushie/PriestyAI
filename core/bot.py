...
from core.ui_components import (
    DynamicView, ThoughtsButton, EphemeralThoughtsView, sanitize_thoughts,
    CustomButton, CustomUserSelect, CustomRoleSelect, CustomChannelSelect,
    CustomStringSelect, CustomModalButton, DynamicModal, ConfigModal, CustomMentionableSelect,
    SaveContextModal, AgentPreStartView, ProfileEditModal
)
from agents.discord_react.agent import AgentSession
from tools.message_builder.mb_tool import build_layout_generator_prompt, build_message_layout
from google.genai import types

from core.voice_client import DiscordVoiceSession
from discord.ext import voice_recv

from tools.message_builder import build_message_layout, inject_message_builder_hook, DSLRuntimeView, DSL_STATE_STORAGE
...
def generate_slug_from_prompt(prompt: str) -> str:
    ...
    stop_words = {
        ...
        "to", "for", "with", "of", "on", "at", "in", "by", "from", 
        "who", "whom", "how", "what", "which", "compare", "determine", "check"
    }
...
def split_outside_parentheses(text: str, char: str = ":", maxsplit: int = -1) -> list[str]:
...
class SubmissionCollector:
    ...
    def __init__(self, message_id: int, channel_id: int, anonymous: bool, timeout: int, prompt_title: str):
...
class FriendBot(discord.Client):
    ...
    async def get_config(self, target_id: int, is_dm: bool) -> dict:
        ...
        if loaded:
            if "discord_tools" in loaded:
                ...
                legacy_selectors = ["Member Selector", "Channel Selector", "Role Selector"]
        ...
        default_cfg = {
            ...
            "tool_mode": "Auto",
            "thinking_level": "Auto",
            "system_tools": ["Google Search", "Code Execution", "URL Content", "Generate Images", "Memory Journals", "Message Builder"],
            "discord_tools": ["Buttons", "Modals", "Threads", "Entity Dropdowns", "Custom Dropdowns", "Double-Texting", "Reactions", "Native Polls", "Unicode Emojis", "Server Emojis"],
            "server_profile": {
                "nickname": "",
                "bio": "",
                "avatar_url": None,
    ...
    async def setup_hook(self):
        ...
        self.tree.add_command(app_commands.ContextMenu(name="Re-run", callback=self.context_rerun_pagination))
        self.tree.add_command(app_commands.ContextMenu(name="Branch", callback=self.context_branch))
        self.tree.add_command(app_commands.ContextMenu(name="Delete Bot Message", callback=self.context_delete))
        self.tree.add_command(app_commands.ContextMenu(name="Save Message as Context", callback=self.context_save_message))
        
        self.tree.add_command(app_commands.ContextMenu(name="Reset AI Memory", callback=self.context_reset_memory))
        self.tree.add_command(app_commands.ContextMenu(name="Save User as Context", callback=self.context_save_user))
        
        @app_commands.command(name="config", description="Configure AI tools, prompt behavior, custom profiles, or Server News")
        ...
        async def config_cmd(
            ...
            target: Literal["Channel Settings", "Server Identity", "Server News"] = "Channel Settings"
        ):
            ...
            if target == "Server Identity":
                ...
                current_name = saved_profile.get("nickname") or bot_member.nick or bot_member.global_name or bot_member.name
                ...
                enabled_guilds = [g.strip() for g in news_enabled_env.split(",") if g.strip()]
        ...
        context_group = app_commands.Group(name="context", description="Manage context snapshots for agent sessions")

        @context_group.command(name="delete", description="Permanently delete a saved context snapshot")
        @app_commands.describe(alias="The lowercase name of the context alias to remove")
        async def context_delete_cmd(interaction: discord.Interaction, alias: str):
            ...
            if not clean_alias:
                await interaction.followup.send("❌ Error: Invalid context alias format.", ephemeral=True)
                return
                
            guild = self.get_guild(self.brain_server_id)
            if not guild:
                await interaction.followup.send("❌ Error: Brain server is unreachable.", ephemeral=True)
                return
                
            forum = discord.utils.get(guild.channels, name="context-snippets", type=discord.ChannelType.forum)
            if not forum:
                await interaction.followup.send("❌ Error: No saved contexts exist.", ephemeral=True)
                return
            ...
            if not thread:
                await interaction.followup.send("❌ Error: You have no saved context records.", ephemeral=True)
                return
            ...
            if deleted:
                await interaction.followup.send(f"✅ Context snapshot `{clean_alias}` has been permanently deleted.", ephemeral=True)
            else:
                await interaction.followup.send(f"⚠️ Context snapshot `{clean_alias}` was not found in your records.", ephemeral=True)

        self.tree.add_command(context_group)

        @app_commands.command(name="agent", description="Start an autonomous diagnostic or research agent (Private Thread)")
        @app_commands.describe(
        ...
        @app_commands.command(name="join", description="Request the AI companion to connect to your current Voice Channel")
        async def join_voice_cmd(interaction: discord.Interaction):
            if not interaction.guild:
                await interaction.response.send_message("❌ Voice commands can only be executed inside a server.", ephemeral=True)
                return
                
            if not interaction.user.voice or not interaction.user.voice.channel:
                await interaction.response.send_message("❌ You must connect to a voice channel before requesting the bot to join.", ephemeral=True)
                return
            ...
            if guild_id in self.voice_sessions:
                await self.voice_sessions[guild_id].stop()
            ...
            try:
                ...
                await target_channel.guild.change_voice_state(channel=target_channel, self_mute=False, self_deaf=False)
                
                await interaction.followup.send(f"✅ Joined and initialized voice call in **#{target_channel.name}**!", ephemeral=True)
                
            except Exception as e:
                logger.error(f"Failed to connect to voice: {e}", exc_info=True)
                await interaction.followup.send(f"❌ An error occurred connecting to voice: `{e}`", ephemeral=True)

        self.tree.add_command(join_voice_cmd)

        @app_commands.command(name="leave", description="Disconnect the AI companion from the current Voice Channel")
        async def leave_voice_cmd(interaction: discord.Interaction):
            if not interaction.guild:
                await interaction.response.send_message("❌ This command is restricted to server guilds.", ephemeral=True)
                return
            ...
            if guild_id in self.voice_sessions:
                await interaction.response.defer(ephemeral=True)
                await self.voice_sessions[guild_id].stop()
                await interaction.followup.send("👋 Successfully disconnected from the voice channel.", ephemeral=True)
            else:
                await interaction.response.send_message("❌ I am not connected to any voice channels in this server.", ephemeral=True)

        self.tree.add_command(leave_voice_cmd)

        @app_commands.command(name="start", description="[Owner-Only] Manually trigger news pre-generation and broadcast pipeline")
        @app_commands.describe(
        ...
        async def start_cmd(
            ...
            if not owner_id_env or interaction.user.id != int(owner_id_env.strip()):
                await interaction.response.send_message("❌ This diagnostic command is restricted to the bot owner.", ephemeral=True)
                return
                
            await interaction.response.defer(ephemeral=True)
            status_msg = await interaction.followup.send("🚀 *Initializing diagnostic news pre-generation...*", wait=True)
            ...
            if not config.get("news_enabled", False):
                await edit_status("❌ [Failure] Server News must be enabled via `/config target:Server News` first.")
                return
            ...
            length_override = length if length is not None else config.get(f"{edition}_length", "Standard")
            
            import core.memory as memory
            state = await memory.load_news_state(self, self.brain_server_id, guild_id)
            if not state:
                state = {
            ...
            if not show_name:
                await edit_status("🎨 *Show branding missing. Generating permanent news station name...*")
                show_name = await self._generate_show_branding(interaction.guild.name, self.gemini_key, os.getenv("GEMINI_NEWS_MODEL", "gemini-2.5-flash"))
            ...
            logger.info(f"[Diagnostic News] Manual run started. Edition: {edition} | Format: {length_override} | Episode: {episode}")

            try:
                await edit_status("⏳ **[Step 1/5]** Scraping server logs, announcements, and schedules...")
                ...
                await edit_status("⏳ **[Step 2/5]** Composing layout scripts and overlays via Gemini API...")
                ...
                await edit_status("⏳ **[Step 3/5]** Rendering video frames & mixing audio (Rendering thread active, please wait ~2-15 mins)...")
                ...
                await edit_status("⏳ **[Step 4/5]** Uploading final render output to Streamable hosting...")
                from tools.news.news_orchestrator import upload_to_streamable
                title = f"{show_name} - Ep. {episode} ({edition.capitalize()})"
                streamable_url = await asyncio.to_thread(upload_to_streamable, local_output_filename, title)
                
                if not streamable_url:
                    await edit_status("❌ **[Failure]** Pre-generation failed: Streamable upload returned an empty URL.")
                    return
                ...
                await edit_status("⏳ **[Step 5/5]** Writing daily highlight summary and broadcasting news...")
                await self._run_broadcast_pipeline(guild_id, edition, config, state)
                
                await edit_status(f"🎉 **Diagnostic News Run Succeeded!**\n📺 Watch output here: {streamable_url}")

            except Exception as err:
                logger.error(f"[Diagnostic News] Manual pre-generation crash: {err}", exc_info=True)
                await edit_status(f"❌ **Diagnostic Run Failed**: `{err}`\nCheck the console log for complete trace details.")

        self.tree.add_command(start_cmd)

        @app_commands.command(name="generate", description="Force the AI to programmatically generate structured outputs")
        @app_commands.describe(
        ...
        async def generate_cmd(interaction: discord.Interaction, type: app_commands.Choice[str], prompt: str):
            ...
            if type.value == "image":
                ...
                placeholder = await interaction.followup.send("🎨 *Generating Image...*", wait=True)
                history = self.history_tracker.get_formatted_history(interaction.channel.id)
                
                self.loop.create_task(
                    self._generate_decoupled_image(
                        channel=interaction.channel, author=interaction.user, raw_image_prompt=prompt,
                        placeholder_msg=placeholder, context_history=history, is_edit_flow=False,
                        original_message=None, banter="", disabled_triggers={}, config=config_override
                    )
                )
                ...
                placeholder = await interaction.followup.send("🎨 *Generating Message Builder Layout...*", wait=True)
                ...
                context = self._compile_server_context(interaction.guild, interaction.user) if interaction.guild else ""
                memories = {"user_memories": "", "server_lore": "", "global_database": ""}
                
                await self._execute_ai_with_retries(
                    override_prompt, history, [], display_name, memories, context,
                    interaction.channel, interaction.user, is_dm, None, edit_target=placeholder, config=config_override
                )
                
            elif type.value == "legacy":
                placeholder = await interaction.followup.send("⚡ *Generating Legacy Interface...*", wait=True)
                ...
                await self._execute_ai_with_retries(
                    override_prompt, history, [], display_name, memories, context,
                    interaction.channel, interaction.user, is_dm, None, edit_target=placeholder, config=active_config
                )
    ...
    async def _compile_selected_context_payloads(self, user_id: int, contexts_str: str) -> str:
        ...
        all_contexts = await memory.fetch_all_contexts_for_user(self, self.brain_server_id, user_id)
        if not all_contexts:
            return ""
        ...
        for payload in matched_payloads:
            ...
            if payload.get("additional_notes"):
                composed_lines.append(f"- Additional Notes: \"{payload.get('additional_notes')}\"")
    ...
    async def _process_context_autocomplete_choices(self, interaction: discord.Interaction, current_input: str) -> list[tuple[str, str]]:
        ...
        if cached:
            ...
            if now - cached[0] > 45.0:
                async def refresh_task():
                    try:
                        latest = await memory.fetch_all_contexts_for_user(self, self.brain_server_id, user_id)
                        self.user_context_cache[user_id] = (time.time(), latest)
                    except Exception as err:
                        logger.warning(f"Silently failed to background-refresh contexts: {err}")
        ...
        try:
            context_choices = [(f"context: {c['alias']}", c['alias']) for c in all_contexts if c.get("alias")]
            ...
            server_choices = [(f"server: {g.name}", f"server_{g.id}") for g in mutual_guilds]
            ...
            available_choices = [(lbl, val) for lbl, val in all_choices if val not in completed_choices]
            
            matches = []
            for lbl, val in available_choices:
                if val.lower().startswith(active_typing.lower()) or lbl.lower().startswith(active_typing.lower()):
    ...
    async def context_save_user(self, interaction: discord.Interaction, member: discord.Member):
        ...
        suggested_alias = re.sub(r'[^a-z0-9_]', '', member.display_name.lower().replace(" ", "_"))
    ...
    async def context_save_message(self, interaction: discord.Interaction, message: discord.Message):
        ...
        author_alias = re.sub(r'[^a-z0-9_]', '', message.author.display_name.lower().replace(" ", "_"))
    ...
    async def _choose_semantic_emoji(self, content: str) -> str:
        ...
        try:
            ...
            if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                emoji = "".join(p.text for p in response.candidates[0].content.parts if getattr(p, 'text', None)).strip()
    ...
    async def _monitor_poll_end(self, poll_msg: discord.Message, delay_seconds: int):
        ...
        try:
            ...
            results_summary = [f"- {answer.text}: {answer.vote_count} votes" for answer in poll.answers]
            results_text = "\n".join(results_summary)
            
            prompt = (f"[System Prompt: The native Discord poll you posted \"{poll.question.text}\" has just closed. "
            ...
            async with channel.typing():
                ...
                await self._execute_ai_with_retries(
                    prompt=prompt, history=history, attachments=[], display_name=target_user.display_name, 
                    memory_dict=memories, context=server_context, channel=channel, author=target_user, 
                    is_dm=isinstance(channel, discord.DMChannel), original_message=refetched_msg
                )
        except Exception as e:
    ...
    async def context_generate_image(self, interaction: discord.Interaction, message: discord.Message):
        ...
        try:
            ...
            self.loop.create_task(
                self._generate_decoupled_image(
                    channel=message.channel, author=message.author, raw_image_prompt=prompt,
                    placeholder_msg=placeholder_msg, context_history=history, is_edit_flow=False,
                    original_message=message, banter="", disabled_triggers=None
                )
            )
        except Exception as e:
    ...
    async def context_rerun_pagination(self, interaction: discord.Interaction, message: discord.Message):
        ...
        if message.reference and message.reference.resolved:
            ...
            try: original_user_msg = await message.channel.fetch_message(message.reference.message_id)
        ...
        async with channel.typing():
            ...
            if "Memory Journals" in config_state.get("system_tools", []):
                ...
                async def get_empty(): return {"user_memories": "", "server_lore": "", "global_database": ""}
            ...
            if message.id not in self.rerun_cache:
                self.rerun_cache[message.id] = [{"content": message.content, "attachments": list(message.attachments), "ui_state": {}}]
                self.rerun_indexes[message.id] = 0

            await self._execute_ai_with_retries(
                prompt=original_user_msg.clean_content, history=history, attachments=list(original_user_msg.attachments),
                display_name=display_name, memory_dict=memories, context=server_context, channel=channel,
                author=original_user_msg.author, is_dm=is_dm, original_message=original_user_msg,
                scraped_pages=scraped_pages, user_status=user_status, edit_target=message, config=config_state
            )
    ...
    async def context_branch(self, interaction: discord.Interaction, message: discord.Message):
        ...
        try:
            ...
            preceding_history = [hist_msg async for hist_msg in message.channel.history(limit=10, before=message)]
            preceding_history.reverse()
            preceding_history.append(message)

            history_str = "\n".join([f"[{h.created_at.strftime('%H:%M:%S')}] {h.author.display_name}: {h.clean_content}" for h in preceding_history])
            ...
            async with thread.typing():
                ...
                await self._execute_ai_with_retries(
                    prompt=branch_prompt, history=history_str, attachments=[], display_name=display_name,
                    memory_dict=memories, context=server_context, channel=thread, author=message.author,
                    is_dm=False, original_message=None, config=config_state
                )
        except Exception as e:
    ...
    async def context_extract_dsl(self, interaction: discord.Interaction, message: discord.Message):
    ...
    async def _presence_monitor_loop(self):
        ...
        while not self.is_closed():
            ...
            if not self.dnd_until:
                ...
                if elapsed_seconds >= 300: await self.change_presence(status=discord.Status.idle)
                else: await self.change_presence(status=discord.Status.online)
    ...
    async def _spontaneous_checkin_loop(self):
        ...
        while not self.is_closed():
            ...
            pt_tz = getattr(getattr(self, 'chat_handler', None), 'pt_zone', timezone.utc)
            now_pt = dt_class.now(pt_tz)
            if now_pt.hour >= 23 or now_pt.hour < 8:
                logger.info(f"Spontaneous check-in suppressed during quiet hours: {now_pt.strftime('%I:%M %p %Z')}")
                continue
            ...
            if elapsed_seconds >= 3600 and self.active_channels:
                if random.random() < 0.30:
                    ...
                    if channel:
                        raw_members = channel.members if hasattr(channel, 'members') else []
                        
                        active_members = []
                        for rm in raw_members:
                            m = getattr(rm, 'member', rm)
                            if m and not getattr(m, 'bot', True):
                                if hasattr(m, 'status') and m.status in (discord.Status.online, discord.Status.idle):
                                    active_members.append(m)
                                    
                        if not active_members:
                            logger.info(f"Spontaneous check-in aborted in channel {target_channel_id}: No active members online.")
                            continue
                        ...
                        async with channel.typing():
                            ...
                            if "Memory Journals" in config_state.get("system_tools", []):
                                memories = await self._compile_memories_for_ai(target_user, channel, query_text=spontaneous_prompt)
                            else:
                                memories = {"user_memories": "", "server_lore": "", "global_database": ""}
                ...
                if channel:
                    try:
                        async with channel.typing():
                            ...
                            await channel.send("alright, heading back to my main threads now. let me know if you need me again")
                    except Exception as decay_err:
                        logger.warning(f"Failed to post decay sign-off to channel {cid}: {decay_err}")

    async def _run_async_memory_evaluation(self, user_msg: str, bot_reply: str, author, channel):
        """
        ...
        logger.info(f"Triggering asynchronous ChatGPT-style memory evaluation for {author.display_name}")
        ...
        try:
            ...
            res_text = ""
            if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                res_text = "".join(p.text for p in response.candidates[0].content.parts if getattr(p, 'text', None)).strip()

            json_match = re.search(r'```json\s*(.*?)\s*```', res_text, flags=re.DOTALL | re.IGNORECASE)
            if json_match:
                ...
                for entry in payload:
                    ...
                    if score < 8:
                        logger.info(f"Memory Gatekeeper ignored noisy fact (Score {score}): '{fact_text}'")
                        continue
                        
                    if action == "LEARN":
                        if target == "USER":
                            await memory.save_fact(self, self.brain_server_id, author, fact_text, category, score)
                            logger.info(f"Async Evaluator: Learned User Fact: '{fact_text}' (Score: {score})")
                        elif target == "SERVER" and hasattr(channel, "guild") and channel.guild:
                            await memory.save_server_fact(self, self.brain_server_id, channel.guild, fact_text, category, score)
                            logger.info(f"Async Evaluator: Learned Server Fact: '{fact_text}' (Score: {score})")
                        elif target == "GLOBAL":
                            await memory.save_global_fact(self, self.brain_server_id, fact_text, category, score)
                            logger.info(f"Async Evaluator: Learned Global Fact: '{fact_text}' (Score: {score})")
                            
                    elif action == "FORGET":
                        if target == "USER":
                            u_channel = f"{author.name}-memory".lower().replace(" ", "-")
                            await memory.forget_fact(self, self.brain_server_id, "🧠 User Memories", u_channel, fact_text)
                        elif target == "SERVER" and hasattr(channel, "guild") and channel.guild:
                            s_channel = f"{channel.guild.name}-lore".lower().replace(" ", "-")
                            await memory.forget_fact(self, self.brain_server_id, "🌍 Server Lore", s_channel, fact_text)
                        elif target == "GLOBAL":
                            await memory.forget_fact(self, self.brain_server_id, "🌐 Global Database", "global-memory", fact_text)
                            
        except Exception as eval_err:
            logger.warning(f"Memory evaluation task failed: {eval_err}")

    async def _compile_memories_for_ai(self, author, channel, query_text: str = "") -> dict:
        """Helper that groups long-term memories using semantic lookup loops."""
        user_channel = f"{author.name}-memory".lower().replace(" ", "-")
        user_task = memory.fetch_memory_block(self, self.brain_server_id, "🧠 User Memories", user_channel, query_text)
        if channel.guild:
            server_channel = f"{channel.guild.name}-lore".lower().replace(" ", "-")
            server_task = memory.fetch_memory_block(self, self.brain_server_id, "🌍 Server Lore", server_channel, query_text)
        else:
            async def get_empty(): return ""
            server_task = get_empty()
            
        global_task = memory.fetch_memory_block(self, self.brain_server_id, "🌐 Global Database", "global-memory", query_text)
    ...
    async def trigger_ai_reply(self, channel, user: discord.Member):
        async with channel.typing():
            ...
            if "Memory Journals" in config_state.get("system_tools", []):
                memories = await self._compile_memories_for_ai(user, channel, query_text=prompt)
            else:
                memories = {"user_memories": "", "server_lore": "", "global_database": ""}
                
            await self._execute_ai_with_retries(prompt, history, [], user.display_name, memories, server_context, channel, user, is_dm, None, config=config_state)

    async def trigger_ephemeral_ai_reply(self, interaction: discord.Interaction, collector: SubmissionCollector, user_choice_str: str):
        ...
        history = self.history_tracker.get_formatted_history(interaction.channel.id)
        display_name = interaction.user.nick if isinstance(interaction.user, discord.Member) and interaction.user.nick else interaction.user.display_name
        server_context = self._compile_server_context(interaction.guild, interaction.user) if interaction.guild else ""
        ...
        if "Memory Journals" in config_state.get("system_tools", []):
            memories = await self._compile_memories_for_ai(interaction.user, interaction.channel, query_text=ephemeral_prompt)
        else:
            memories = {"user_memories": "", "server_lore": "", "global_database": ""}
        
        await self._execute_ai_with_retries(
            prompt=ephemeral_prompt, history=history, attachments=[], display_name=display_name,
            memory_dict=memories, context=server_context, channel=interaction.channel, author=interaction.user,
            is_dm=is_dm, original_message=None, ephemeral_interaction=interaction, config=config_state
        )
    ...
    async def _run_collection_timeout(self, message_id: int, duration_seconds: int):
        ...
        channel = self.get_channel(collector.channel_id) or await self.fetch_channel(collector.channel_id)
        ...
        for i, sub in enumerate(collector.submissions):
            user_label = "Anonymous Friend" if collector.anonymous else f"{sub['display_name']} (@{sub['username']})"
            submission_lines.append(f"Submission {i+1} by {user_label}: {sub['data']}")
            
        compiled_data = "\n".join(submission_lines) if submission_lines else "No submissions were recorded."
        participants_str = ", ".join([f"<@{uid}>" for uid in collector.participants]) if collector.participants else "No one"
        ...
        async with channel.typing():
            ...
            history = self.history_tracker.get_formatted_history(channel.id)
            server_context = self._compile_server_context(channel.guild, None) if hasattr(channel, 'guild') else ""
            ...
            if "Memory Journals" in config_state.get("system_tools", []):
                memories = await self._compile_memories_for_ai(target_user, channel, query_text=compilation_prompt)
            else:
                memories = {"user_memories": "", "server_lore": "", "global_database": ""}
            
            await self._execute_ai_with_retries(
                prompt=compilation_prompt, history=history, attachments=[], display_name=self.user.display_name,
                memory_dict=memories, context=server_context, channel=channel, author=target_user,
        ...
        try:
            ...
            if message.author.id == self.user.id:
                async with channel.typing():
                    ...
                    context_history = ""
                    try:
                        surrounding = [m async for m in channel.history(limit=5, before=message)]
                        surrounding.reverse()
                        context_lines = []
                        for m in surrounding:
                            author_lbl = m.author.nick if isinstance(m.author, discord.Member) and m.author.nick else m.author.display_name
                            context_lines.append(f"[{m.created_at.strftime('%H:%M:%S')}] {author_lbl}: {m.clean_content}")
                        context_history = "\n".join(context_lines)
                    except Exception as context_err:
                        logger.warning(f"Could not fetch surrounding context history for reaction: {context_err}")
                    
                    self.history_tracker.add_system_action(channel.id, f"{display_name} reacted to my message with {payload.emoji.name}")
                    ...
                    history = self.history_tracker.get_formatted_history(channel.id)
                    server_context = self._compile_server_context(channel.guild, user) if channel.guild else "Environment: Direct Messages."
                    ...
                    await self._execute_ai_with_retries(
                        prompt=prompt, history=history, attachments=[], display_name=display_name, 
                        memory_dict=memories, context=server_context, channel=channel, author=user, 
                        is_dm=is_dm, original_message=message, config=config_state
                    )
        except Exception as e:
            ...
            if message.author.id == self.user.id:
                async with channel.typing():
                    ...
                    try:
                        ...
                        logger.warning(f"Could not fetch surrounding context history for reaction remove: {context_err}")
                        
                    self.history_tracker.add_system_action(channel.id, f"{display_name} removed their reaction {payload.emoji.name} from my message")
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.channel.id in self.active_agent_sessions:
            ...
            if session.status == "paused_user_question":
                ...
                session.react_history[-1]["observation"] = f"User replied via text: '{message.clean_content}'"
                self.loop.create_task(session.execute_tick(self))
                return

        is_thread = isinstance(message.channel, discord.Thread)
        is_run_trigger = "run:" in message.content.lower() or (self.user in message.mentions and "run" in message.content.lower())
        
        if is_thread and is_run_trigger and message.channel.id not in self.active_agent_sessions:
            prompt_content = ""
            ...
            if prompt_content:
                ...
                checklist_msg = await message.channel.send(content=checklist_content, view=view)
        ...
        if message.guild and message.guild.me:
            is_role_mentioned = any(role in message.role_mentions for role in message.guild.me.roles)

        is_watched = message.channel.id in self.active_channels
        is_dm = isinstance(message.channel, discord.DMChannel)

        if is_mentioned or is_reply_to_bot or is_watched or is_dm or is_role_mentioned or is_everyone_mentioned:
            if is_everyone_mentioned and not (is_mentioned or is_reply_to_bot or is_role_mentioned):
                if random.random() < 0.90:
                    try:
                        chosen_emoji = await self._choose_semantic_emoji(message.clean_content)
            ...
            async with message.channel.typing():
                ...
                if "Generate Images" not in config_state.get("system_tools", []):
                    if "[IMAGE_PENDING:" in raw_text_to_scan or "[IMAGE:" in raw_text_to_scan or any(kw in raw_text_to_scan.lower() for kw in ["draw me", "generate an image of", "paint me"]):
                ...
                if "Memory Journals" in config_state.get("system_tools", []):
                    memories_task = self._compile_memories_for_ai(message.author, message.channel, query_text=prompt)
                else:
                    async def get_empty_mem(): return {"user_memories": "", "server_lore": "", "global_database": ""}
                ...
                await self._execute_ai_with_retries(
                    prompt=prompt, history=history, attachments=all_attachments, display_name=display_name, 
                    memory_dict=memories, context=server_context, channel=message.channel, author=message.author, 
                    is_dm=is_dm, original_message=message, scraped_pages=scraped_pages, user_status=user_status,
                    config=config_state, disabled_triggers=disabled_triggers
                )

    def format_thoughts_block(self, thoughts: str, thinking_active: bool = False, thinking_level: str = "HIGH") -> str:
    ...
    async def _send_split_content(self, channel, content: str, view=None, edit_target=None, original_message=None):
        ...
        if edit_target:
            if file_arg:
                sent_msg = await edit_target.edit(content=chunks[0], view=view, attachments=file_arg)
            else:
                sent_msg = await edit_target.edit(content=chunks[0], view=view)
                
            for chunk in chunks[1:]:
                await channel.send(content=chunk)
        elif original_message and (self.user in original_message.mentions or original_message.reference):
            if file_arg:
                sent_msg = await original_message.reply(content=chunks[0], view=view, files=file_arg)
            else:
            ...
            if file_arg:
                sent_msg = await channel.send(content=chunks[0], view=view, files=file_arg)
            else:
    ...
    async def _render_math_from_text(self, text: str) -> Tuple[Optional[discord.File], str]:
        """
        ...
        if not math_string:
            ...
            for pattern in plain_patterns:
                ...
                if match:
                    ...
                    if any(char in candidate for char in ["\\", "^", "_", "(", ")", "/", "*", "=", "π"]):
                ...
                async with session.get(url, timeout=10) as response:
                    if response.status == 200:
                        ...
                        if img_bytes and len(img_bytes) > 100:
                            file = discord.File(fp=io.BytesIO(img_bytes), filename="equation.png")
                            clean_text = text.replace(matched_block, f"**{math_string_clean}**")
    ...
    async def _execute_ai_with_retries(self, prompt, history, attachments, display_name, memory_dict, context, channel, author, is_dm, original_message, scraped_pages=None, user_status=None, edit_target=None, ephemeral_interaction: discord.Interaction = None, config: dict = None, disabled_triggers: dict = None, self_correction_count: int = 0):
        max_retries = 6
        ...
        if config_thinking == "Auto":
            current_thinking_level = self.chat_handler._select_thinking_level(prompt, history)
        ...
        should_think = (current_thinking_level in ("HIGH", "MINIMAL")) and (ephemeral_interaction is None)
        user_app_session_id = config.get("user_app_session_id")
        
        if should_think:
            bot_name = channel.guild.me.display_name if channel.guild and channel.guild.me else self.user.display_name
            initial_text = f"💭 *{bot_name} is thinking...*"
            initial_view = DynamicView(self, channel, bot_text=initial_text, search_queries=None, code_blocks=None, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
            initial_btn = ThoughtsButton("Thinking has just started...", elapsed=0, thinking_active=True, message_id=0, bot_instance=self, thinking_level=current_thinking_level)
            initial_view.add_item(initial_btn)
            initial_view.finalize_layout()
            
            if not edit_target:
                if original_message and (self.user in original_message.mentions or original_message.reference):
                    edit_target = await original_message.reply(content=initial_text, view=initial_view)
                else:
                    edit_target = await channel.send(content=initial_text, view=initial_view)
            else:
                try:
                    await edit_target.edit(content=initial_text, view=initial_view, attachments=[])
                except Exception as e:
                    logger.warning(f"Failed to edit target message with placeholder: {e}")
        ...
        for attempt in range(max_retries):
            try:
                response_stream = await self.chat_handler.generate_reply_stream(
                    ...
                    scraped_pages=scraped_pages, user_status=user_status, thinking_level=current_thinking_level,
                    is_dm=is_dm, active_config=config
                )
                await self._process_stream_and_send(
                    response_stream=response_stream, channel=channel, author=author, is_dm=is_dm, 
                    original_message=original_message, edit_target=edit_target, should_think=should_think, 
                    thinking_level=current_thinking_level, ephemeral_interaction=ephemeral_interaction,
                    disabled_triggers=disabled_triggers, config=config, self_correction_count=self_correction_count,
                    prompt=prompt, history=history, attachments=attachments, display_name=display_name,
                    memory_dict=memory_dict, context=context, scraped_pages=scraped_pages, user_status=user_status
                )
                ...
                if "429" in error_str or "exhausted" in error_str or "quota" in error_str:
                    if not self.chat_handler.premium_cooldown_until:
                        logger.warning("Premium model quota exhausted! Placing premium model on PT midnight cooldown and immediately retrying with Fallback.")
                        tomorrow_pt = now_pt + timedelta(days=1)
                        self.chat_handler.premium_cooldown_until = dt_class.combine(
                            tomorrow_pt.date(), datetime.time(0, 0, 0), tzinfo=self.chat_handler.pt_zone
                        )
                        await asyncio.sleep(1.0)
                        continue

                if "429" in error_str and not ("exhausted" in error_str or "quota" in error_str):
                ...
                if ("500" in error_str or "503" in error_str or "internal" in error_str or "unavailable" in error_str):
                    logger.warning("Server error detected! Changing status to DND while retrying...")
                    self.dnd_until = dt_class.now(timezone.utc) + timedelta(minutes=2)
                    await self.change_presence(status=discord.Status.dnd)
                
                if attempt == max_retries - 1:
                    excuse_msg = random.choice([
                        "wait my discord is lagging so bad rn, hold on", "bruh my internet is literally dying, brb",
                        "sorry my connection is acting up lol, what did u say again?", "hold on my brain is literally lagging rn, brb"
                    ])
                    excuse_view = DynamicView(self, channel, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
                    excuse_view.finalize_layout()
                    
                    if ephemeral_interaction:
                        try:
                            if ephemeral_interaction.is_expired():
                                await ephemeral_interaction.followup.send(content=excuse_msg, ephemeral=True)
                            else:
                                await ephemeral_interaction.response.send_message(content=excuse_msg, ephemeral=True)
                        except Exception: pass
                    elif edit_target: await edit_target.edit(content=excuse_msg, view=excuse_view)
                    elif original_message: await original_message.reply(content=excuse_msg, view=excuse_view)
                    else: await channel.send(content=excuse_msg, view=excuse_view)
                    self.dnd_until = None
                    await self.change_presence(status=discord.Status.online)
                    return
    ...
    async def _process_stream_and_send(self, response_stream, channel, author, is_dm, original_message, edit_target=None, should_think=False, thinking_level="HIGH", ephemeral_interaction: discord.Interaction = None, disabled_triggers: dict = None, config: dict = None, self_correction_count: int = 0, prompt=None, history=None, attachments=None, display_name=None, memory_dict=None, context=None, scraped_pages=None, user_status=None):
        ...
        async def ticker_loop():
            ...
            while not generation_complete:
                ...
                for thought_match in re.finditer(r'(?i)<\s*thought\b[^>]*>?\s*(.*?)(?:<\s*/\s*thought\s*>|$)', combined_raw, flags=re.DOTALL):
                    extracted_live_thoughts.append(thought_match.group(1).strip())
                if extracted_live_thoughts:
                    current_thoughts_str += "\n\n" + "\n\n".join(extracted_live_thoughts)
                current_thoughts_str = current_thoughts_str.strip()
                
                view = DynamicView(self, channel, search_queries=search_queries, code_blocks=code_blocks, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
                thoughts_payload = current_thoughts_str if current_thoughts_str else "Thinking has just started..."
                sanitized_payload = sanitize_thoughts(thoughts_payload)
                
                btn = ThoughtsButton(sanitized_payload, elapsed=current_elapsed, thinking_active=True, message_id=sent_msg.id, bot_instance=self, thinking_level=thinking_level)
                view.add_item(btn)
                ...
                for listener_data in list(self.active_thought_listeners.get(sent_msg.id, [])):
                    try:
                        ephemeral_view = EphemeralThoughtsView(sanitized_payload, thinking_active=True, current_level=thinking_level, page_index=listener_data["page_index"], bot_instance=self)
                        await listener_data["message"].edit(view=ephemeral_view)
                    except Exception:
                        if listener_data in self.active_thought_listeners[sent_msg.id]:
                            self.active_thought_listeners[sent_msg.id].remove(listener_data)

        if should_think:
            initial_text = f"💭 *{bot_name} is thinking...*"
            view = DynamicView(self, channel, search_queries=None, code_blocks=None, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
            btn = ThoughtsButton("Thinking has just started...", elapsed=0, thinking_active=True, message_id=0, bot_instance=self, thinking_level=thinking_level)
            view.add_item(btn)
            view.finalize_layout()
            
            if not sent_msg:
                if original_message and (self.user in original_message.mentions or original_message.reference):
                    sent_msg = await original_message.reply(content=initial_text, view=view)
                else:
                    ...
                    if chunk.candidates and chunk.candidates[0].content and chunk.candidates[0].content.parts:
                        for part in chunk.candidates[0].content.parts:
                            if getattr(part, 'executable_code', None):
                                ...
                                if code_blocks:
                                    code_blocks[-1]["output"] = part.code_execution_result.output
                                    
                    if chunk.candidates and chunk.candidates[0].grounding_metadata:
                        meta = chunk.candidates[0].grounding_metadata
                        if getattr(meta, 'web_search_queries', None): search_queries.extend(meta.web_search_queries)
                        
                    if chunk.candidates and chunk.candidates[0].content and chunk.candidates[0].content.parts:
                        for part in chunk.candidates[0].content.parts:
                            is_thought = getattr(part, 'thought', False)
                            if is_thought and getattr(part, 'text', None): accumulated_thoughts.append(part.text)
                            elif getattr(part, 'text', None): accumulated_text.append(part.text)
        except Exception as e:
        ...
        regex_thoughts = []
        for match in re.finditer(r'(?i)<\s*thought\b[^>]*>?\s*(.*?)(?:<\s*/\s*thought\s*>|$)', final_text_str, flags=re.DOTALL):
            regex_thoughts.append(match.group(1).strip())
        final_text_str = re.sub(r'(?i)<\s*thought\b[^>]*>?\s*.*?(?:<\s*/\s*thought\s*>|$)', '', final_text_str, flags=re.DOTALL)
        
        for match in re.finditer(r'(?i)\[\s*thought\b[^\]]*\]?\s*(.*?)(?:\[\s*/\s*thought\s*\]|$)', final_text_str, flags=re.DOTALL):
            regex_thoughts.append(match.group(1).strip())
        final_text_str = re.sub(r'(?i)\[\s*thought\b[^\]]*\]?\s*.*?(?:\[\s*/\s*thought\s*\]|$)', '', final_text_str, flags=re.DOTALL)
        
        for match in re.finditer(r'(?i)THOUGHT:(.*?)(?=\n\n|\Z)', final_text_str, flags=re.DOTALL | re.IGNORECASE):
            regex_thoughts.append(match.group(1).strip())
        final_text_str = re.sub(r'(?i)THOUGHT:.*?(?=\n\n|\Z)', '', final_text_str, flags=re.DOTALL | re.IGNORECASE)
        
        if regex_thoughts: final_thoughts_str += "\n\n" + "\n\n".join(regex_thoughts)
        ...
        if sent_msg and sent_msg.id in self.active_thought_listeners:
            for listener_data in self.active_thought_listeners[sent_msg.id]:
                try:
                    completed_view = EphemeralThoughtsView(final_thoughts_str, thinking_active=False, current_level=thinking_level, page_index=listener_data["page_index"], bot_instance=self)
        ...
        dsl_instructions_check = "[BUILD_MESSAGE:" in final_text_str or "[BUTTON:" in final_text_str
        if not final_text_str.strip() and not dsl_instructions_check and self_correction_count < 2:
            logger.info("AI returned empty text content on interaction. Triggering fallback conversational correction...")
            fallback_prompt = (
            ...
            await self._execute_ai_with_retries(
                prompt=fallback_prompt, history=history, attachments=attachments,
                display_name=display_name, memory_dict=memory_dict, context=context,
                channel=channel, author=author, is_dm=is_dm, original_message=original_message,
                scraped_pages=scraped_pages, user_status=user_status, edit_target=sent_msg,
                config=fallback_config, self_correction_count=self_correction_count + 1
            )
            return

        await self._process_stream_and_send_final_hub(
            final_text_str=final_text_str, search_queries=search_queries, code_blocks=code_blocks,
            final_thoughts_str=final_thoughts_str, channel=channel, author=author, is_dm=is_dm,
            original_message=original_message, sent_msg=sent_msg, elapsed_total=elapsed_total,
            should_think=should_think, thinking_level=thinking_level, ephemeral_interaction=ephemeral_interaction,
            disabled_triggers=disabled_triggers, config=config, self_correction_count=self_correction_count,
            prompt=prompt, history=history, attachments=attachments, display_name=display_name,
            memory_dict=memory_dict, context=context, scraped_pages=scraped_pages, user_status=user_status
        )

    async def _process_stream_and_send_final_hub(self, final_text_str, search_queries, code_blocks, final_thoughts_str, channel, author, is_dm, original_message, sent_msg, elapsed_total, should_think, thinking_level, ephemeral_interaction=None, disabled_triggers=None, config=None, self_correction_count=0, prompt=None, history=None, attachments=None, display_name=None, memory_dict=None, context=None, scraped_pages=None, user_status=None):
        await self._process_and_send_final(
            response_text=final_text_str, search_queries=search_queries, code_blocks=code_blocks, 
            thoughts_text=final_thoughts_str, channel=channel, author=author, is_dm=is_dm, 
            original_message=original_message, edit_target=sent_msg, elapsed=elapsed_total, 
            should_think=should_think, thinking_level=thinking_level, ephemeral_interaction=ephemeral_interaction,
            disabled_triggers=disabled_triggers, config=config, self_correction_count=self_correction_count,
            prompt=prompt, history=history, attachments=attachments, display_name=display_name,
            memory_dict=memory_dict, context=context, scraped_pages=scraped_pages, user_status=user_status
    ...
    def _populate_dynamic_components(self, view, target_channel, buttons_parsed, user_selects_parsed, role_selects_parsed, channel_selects_parsed, mentionable_selects_parsed, string_selects_parsed, modal_buttons_parsed):
        ...
        for label, color, emoji_char in buttons_parsed:
            if btn_count >= 5:
                btn_row += 1
                btn_count = 0
            btn = CustomButton(label=label, style_name=color, emoji=emoji_char, bot_instance=self, channel=target_channel, row=btn_row)
            view.add_item(btn)
            btn_count += 1
        ...
        for pr in user_selects_parsed:
            view.add_item(CustomUserSelect(placeholder=pr, bot_instance=self, channel=target_channel, row=select_row))
            select_row += 1
            
        for pr in role_selects_parsed:
            view.add_item(CustomRoleSelect(placeholder=pr, bot_instance=self, channel=target_channel, row=select_row))
            select_row += 1
            
        for pr in channel_selects_parsed:
            view.add_item(CustomChannelSelect(placeholder=pr, bot_instance=self, channel=target_channel, row=select_row))
            select_row += 1

        for pr in mentionable_selects_parsed:
            view.add_item(CustomMentionableSelect(placeholder=pr, bot_instance=self, channel=target_channel, row=select_row))
            select_row += 1
            
        for placeholder, options in string_selects_parsed:
            select_options = []
            for o_label, o_desc, o_emoji in options:
                select_options.append(discord.SelectOption(label=o_label, description=o_desc, emoji=o_emoji))
            view.add_item(CustomStringSelect(placeholder=placeholder, options_list=select_options, bot_instance=self, channel=target_channel, row=select_row))
            select_row += 1
        ...
        for btn_label, fields in modal_buttons_parsed:
            if modal_count >= 5:
                modal_row += 1
                modal_count = 0
            btn = CustomModalButton(label=btn_label, title=btn_label, fields=fields, bot_instance=self, channel=target_channel, row=modal_row)
            view.add_item(btn)
            modal_count += 1

    async def _process_and_send_final(self, response_text: str, search_queries: list, code_blocks: list, thoughts_text: str, channel, author, is_dm: bool, original_message: discord.Message = None, edit_target: discord.Message = None, elapsed: int = None, should_think: bool = False, thinking_level: str = "HIGH", ephemeral_interaction: discord.Interaction = None, disabled_triggers: dict = None, config: dict = None, self_correction_count: int = 0, prompt=None, history=None, attachments=None, display_name=None, memory_dict=None, context=None, scraped_pages=None, user_status=None):
        if not response_text and not search_queries and not code_blocks and not thoughts_text: return
        target_channel = channel
        sent_msg = edit_target
        
        thread_match = re.search(r'(?i)\[THREAD(?::\s*(.+?))?\]', response_text)
        thread_title = thread_match.group(1).strip() if (thread_match and thread_match.group(1)) else None
        
        is_already_thread = isinstance(channel, discord.Thread)
        is_too_short = len(response_text) < 800 and not thread_title
        should_create_thread = bool(thread_title) and not is_dm and not is_already_thread and not is_too_short
        ...
        if should_create_thread:
            try:
                thread = await channel.create_thread(name=thread_title, auto_archive_duration=1440, type=discord.ChannelType.public_thread)
                self.active_channels.add(thread.id)
                self.watched_channels_decay[thread.id] = time.time() + 600.0
                logger.info(f"Thread '{thread_title}' successfully created (ID: {thread.id})")
            except Exception as e:
        ...
        if "Message Builder" in config.get("system_tools", []):
            active_text_payload, dsl_instructions = extract_build_message(active_text_payload)
            if thread: thread_text_payload = active_text_payload
            else: main_text_payload = active_text_payload
        else:
            dsl_instructions = None

        if "[RESET_CHAT]" in active_text_payload:
            if channel.id in self.history_tracker.histories: self.history_tracker.histories[channel.id].clear()
            active_text_payload = active_text_payload.replace("[RESET_CHAT]", "").strip()

        if "[CLEAR_WEBPAGE_CACHE]" in active_text_payload:
            active_text_payload = active_text_payload.replace("[CLEAR_WEBPAGE_CACHE]", "").strip()

        for match in re.finditer(r'\[WATCH_CHANNEL:\s*<#?(\d+)>?\s*\]', active_text_payload):
            cid = int(match.group(1))
            self.active_channels.add(cid)
            self.watched_channels_decay[cid] = time.time() + 600.0
            active_text_payload = active_text_payload.replace(match.group(0), "").strip()
        ...
        if "[UNWATCH_CHANNEL]" in active_text_payload:
            ...
            active_text_payload = active_text_payload.replace("[UNWATCH_CHANNEL]", "").strip()
        ...
        if not is_edit_flow:
            for match in re.finditer(r'\[IMAGE_PENDING:\s*(.+?)\]', active_text_payload):
                image_prompts.append(match.group(1).strip())
                active_text_payload = active_text_payload.replace(match.group(0), "")
        ...
        typo_edit_data = None
        for match in re.finditer(r'\[TYPO_EDIT:\s*(.+?)\s*\|\s*(.+?)\]', active_text_payload):
            typo_word, corrected_word = match.groups()
            if typo_word.strip().lower() != corrected_word.strip().lower(): typo_edit_data = (typo_word.strip(), corrected_word.strip())
            active_text_payload = active_text_payload.replace(match.group(0), "")
        ...
        if "[FOLLOW_UP]" in active_text_payload:
            ...
            for p in parts[1:]:
                if p.strip(): followups.append(p.strip())

        for match in re.finditer(r'\[LEARN:\s*(.+?)\]', active_text_payload):
            await memory.save_fact(self, self.brain_server_id, author, match.group(1).strip())
            active_text_payload = active_text_payload.replace(match.group(0), "")

        for match in re.finditer(r'\[FORGET:\s*(.+?)\]', active_text_payload):
            channel_name = f"{author.name}-memory".lower().replace(" ", "-")
            await memory.forget_fact(self, self.brain_server_id, "🧠 User Memories", channel_name, match.group(1).strip())
            active_text_payload = active_text_payload.replace(match.group(0), "")

        for match in re.finditer(r'\[LEARN_SERVER:\s*(.+?)\]', active_text_payload):
            if channel.guild: await memory.save_server_fact(self, self.brain_server_id, channel.guild, match.group(1).strip())
            active_text_payload = active_text_payload.replace(match.group(0), "")

        for match in re.finditer(r'\[FORGET_SERVER:\s*(.+?)\]', active_text_payload):
            if channel.guild:
                channel_name = f"{channel.guild.name}-lore".lower().replace(" ", "-")
                await memory.forget_fact(self, self.brain_server_id, "🌍 Server Lore", channel_name, match.group(1).strip())
            active_text_payload = active_text_payload.replace(match.group(0), "")

        for match in re.finditer(r'\[LEARN_GLOBAL:\s*(.+?)\]', active_text_payload):
            await memory.save_global_fact(self, self.brain_server_id, match.group(1).strip())
            active_text_payload = active_text_payload.replace(match.group(0), "")

        for match in re.finditer(r'\[FORGET_GLOBAL:\s*(.+?)\]', active_text_payload):
            await memory.forget_fact(self, self.brain_server_id, "🌐 Global Database", "global-memory", match.group(1).strip())
            active_text_payload = active_text_payload.replace(match.group(0), "")

        collect_session = None
        for match in re.finditer(r'\[COLLECT:\s*([^|\]]+?)\s*\|\s*([^|\]]+?)\s*(?:\|\s*(anonymous|public))?\s*\]', active_text_payload):
            title = match.group(1).strip()
            duration_raw = match.group(2).strip()
            is_anon = match.group(3).strip().lower() != "public" if match.group(3) else True
            ...
            if dur_match:
                ...
                if unit == 's': duration_sec = val
                elif unit == 'm': duration_sec = val * 60
                elif unit == 'h': duration_sec = val * 3600
                
            collect_session = (title, duration_sec, is_anon)
            active_text_payload = active_text_payload.replace(match.group(0), "")

        buttons_parsed = []
        for match in re.finditer(r'\[BUTTON:\s*([^|\]]+?)\s*\|\s*([^|\]]+?)\s*(?:\|\s*([^\]]+?))?\s*\]', active_text_payload):
        ...
        if "Entity Dropdowns" in disc_tools:
            for match in re.finditer(r'\[USER_SELECT:\s*([^\]]+?)\s*\]', active_text_payload):
                prompt = match.group(1).strip()
                user_selects_parsed.append(prompt)
                active_text_payload = active_text_payload.replace(match.group(0), "")

            for match in re.finditer(r'\[ROLE_SELECT:\s*([^\]]+?)\s*\]', active_text_payload):
                prompt = match.group(1).strip()
                role_selects_parsed.append(prompt)
                active_text_payload = active_text_payload.replace(match.group(0), "")

            for match in re.finditer(r'\[CHANNEL_SELECT:\s*([^\]]+?)\s*\]', active_text_payload):
                prompt = match.group(1).strip()
                channel_selects_parsed.append(prompt)
                active_text_payload = active_text_payload.replace(match.group(0), "")

            for match in re.finditer(r'\[MENTIONABLE_SELECT:\s*([^\]]+?)\s*\]', active_text_payload):
                prompt = match.group(1).strip()
                mentionable_selects_parsed.append(prompt)
                active_text_payload = active_text_payload.replace(match.group(0), "")

        string_selects_parsed = []
        for match in re.finditer(r'\[SELECT_STRING:\s*([^|\]]+?)\s*\|\s*([^\]]+?)\s*\]', active_text_payload):
            ...
            for raw_opt in raw_options:
                ...
                opt_desc = parts[1].strip() if len(parts) > 1 and parts[1].strip() else None
                opt_emoji = parts[2].strip() if len(parts) > 2 and parts[2].strip() else None
                options.append((opt_label, opt_desc, opt_emoji))
            string_selects_parsed.append((placeholder, options))
            active_text_payload = active_text_payload.replace(match.group(0), "")

        modal_buttons_parsed = []
        for match in re.finditer(r'\[MODAL_BUTTON:\s*([^|\]]+?)\s*\|\s*([^\]]+?)\s*\]', active_text_payload):
            ...
            for raw_field in raw_fields:
                ...
                field_style = parts[1].strip() if len(parts) > 1 and parts[1].strip() else "short"
                field_desc = parts[2].strip() if len(parts) > 2 and parts[2].strip() else None
        ...
        has_ui = should_think or bool(search_queries) or bool(code_blocks) or bool(thoughts_text) or bool(buttons_parsed) or bool(user_selects_parsed) or bool(role_selects_parsed) or bool(channel_selects_parsed) or bool(mentionable_selects_parsed) or bool(string_selects_parsed) or bool(modal_buttons_parsed) or bool(user_app_session_id)
        history = self.history_tracker.get_formatted_history(channel.id)

        if ephemeral_interaction:
            view = DynamicView(self, channel, search_queries=search_queries, code_blocks=code_blocks, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
            self._populate_dynamic_components(view, channel, buttons_parsed, user_selects_parsed, role_selects_parsed, channel_selects_parsed, mentionable_selects_parsed, string_selects_parsed, modal_buttons_parsed)
            view.finalize_layout()
            
            main_text_payload = await self._resolve_mentions(main_text_payload, channel)
            try:
                if ephemeral_interaction.is_expired():
                    await ephemeral_interaction.followup.send(content=main_text_payload, view=view, ephemeral=True)
                else:
                    await ephemeral_interaction.response.send_message(content=main_text_payload, view=view, ephemeral=True)
            except Exception as e:
                logger.error(f"Failed to transmit ephemeral interaction response: {e}")
            return

        if thread:
            redirect_view = DynamicView(self, channel, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
            redirect_view.finalize_layout()
            
            main_text_payload = await self._resolve_mentions(main_text_payload, channel)
            if edit_target: await edit_target.edit(content=main_text_payload, embed=None, view=redirect_view)
            elif original_message and (self.user in original_message.mentions or original_message.reference): await original_message.reply(content=main_text_payload, view=redirect_view)
            else: await channel.send(content=main_text_payload, view=redirect_view)
                    
            thread_text = f"hey <@{author.id}>, let's look at this here!\n\n{thread_text_payload}"
            if image_prompts: thread_text += "\n\n*(🎨 Generating Image...)*"
            
            thread_view = DynamicView(self, thread, search_queries=search_queries, code_blocks=code_blocks, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
            if thoughts_text and should_think:
                thread_view.add_item(ThoughtsButton(sanitize_thoughts(thoughts_text), elapsed=elapsed, thinking_active=False, message_id=None, bot_instance=self, thinking_level=thinking_level))
            self._populate_dynamic_components(thread_view, thread, buttons_parsed, user_selects_parsed, role_selects_parsed, channel_selects_parsed, mentionable_selects_parsed, string_selects_parsed, modal_buttons_parsed)
            thread_view.finalize_layout()
            sent_msg = await self._send_split_content(thread, thread_text, view=thread_view)
            target_channel = thread 
            
            if collect_session and sent_msg:
                title, dur, anon = collect_session
                self.active_collectors[sent_msg.id] = SubmissionCollector(sent_msg.id, target_channel.id, anon, dur, title)
                self.loop.create_task(self._run_collection_timeout(sent_msg.id, dur))
            
            if image_prompts:
                self.loop.create_task(
                    self._generate_decoupled_image(
                        channel=target_channel, author=author, raw_image_prompt=image_prompts[0],
                        placeholder_msg=sent_msg, context_history=history, is_edit_flow=is_edit_flow,
                        original_message=original_message, banter=thread_text_payload,
                        disabled_triggers=disabled_triggers, config=config_state
                    )
                )
        else:
            initial_text = main_text_payload
            if image_prompts: initial_text += "\n\n*(🎨 Generating Image...)*"

            if dsl_instructions:
                placeholder_text = "🎨 *Priesty is drafting composition...*"
                placeholder_view = DynamicView(self, target_channel, user_app_session_id=user_app_session_id)
                placeholder_btn = ThoughtsButton("Visualizing layout...", elapsed=0, thinking_active=True, message_id=0, bot_instance=self, thinking_level="HIGH")
                placeholder_view.add_item(placeholder_btn)
                placeholder_view.finalize_layout()
                
                if edit_target:
                    placeholder_msg = await edit_target.edit(content=placeholder_text, view=placeholder_view)
                elif original_message and (self.user in original_message.mentions or original_message.reference):
                    placeholder_msg = await original_message.reply(content=placeholder_text, view=placeholder_view)
                else:
                    placeholder_msg = await target_channel.send(content=placeholder_text, view=placeholder_view)
                ...
                self.loop.create_task(
                    self._generate_decoupled_layout(
                        channel=target_channel, author=author, dsl_instructions=dsl_instructions,
                        placeholder_msg=placeholder_msg, context_history=history, initial_prompt=prompt,
                        config_state=config_state, user_app_session_id=user_app_session_id,
                ...
                if not has_ui and not image_prompts:
                    sent_msg = await self._send_split_content(target_channel, initial_text, original_message=original_message)
                    ...
                    view = DynamicView(self, channel, search_queries=search_queries, code_blocks=code_blocks, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
                    if thoughts_text and should_think:
                        view.add_item(ThoughtsButton(sanitize_thoughts(thoughts_text), elapsed=elapsed, thinking_active=False, message_id=edit_target.id if edit_target else None, bot_instance=self, thinking_level=thinking_level))
                    ...
                    sent_msg = await self._send_split_content(target_channel, initial_text, view=view, edit_target=edit_target, original_message=original_message)
                    ...
                    if image_prompts and sent_msg:
                        self.loop.create_task(
                            self._generate_decoupled_image(
                                channel=target_channel, author=author, raw_image_prompt=image_prompts[0],
                                placeholder_msg=sent_msg, context_history=history, is_edit_flow=is_edit_flow,
                                original_message=original_message, banter=main_text_payload,
                                disabled_triggers=disabled_triggers, config=config_state
                            )
                        )
        ...
        if typo_edit_data and sent_msg: self.loop.create_task(self._apply_typo_edit(sent_msg, typo_edit_data))
        ...
        if followups:
            for text in followups:
                ...
                async with target_channel.typing():
                    await asyncio.sleep(1.5)
                    followup_view = DynamicView(self, target_channel, bot_text=text, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
                    followup_view.finalize_layout()
                    await self._send_split_content(target_channel, text, view=followup_view)

    async def _stream_layout_and_get_params(self, raw_prompt, context_history, channel, placeholder_msg):
        ...
        stream = await self.chat_handler.client.aio.models.generate_content_stream(
            model=self.chat_handler.premium_model,
            contents=parts,
            config=config
        )
        ...
        async for chunk in stream:
            ...
            if now - last_edit >= 1.8:
                ...
                current_thoughts_str = "".join(accumulated_thoughts).strip()
                sanitized_payload = sanitize_thoughts(current_thoughts_str) if current_thoughts_str else "Visualizing layout..."
                
                view = DynamicView(self, channel)
                btn = ThoughtsButton(sanitized_payload, elapsed=current_elapsed, thinking_active=True, message_id=placeholder_msg.id, bot_instance=self, thinking_level="HIGH")
                view.add_item(btn)
                view.finalize_layout()
                ...
                for listener_data in list(listeners):
                    try:
                        ephemeral_view = EphemeralThoughtsView(sanitized_payload, thinking_active=True, current_level="HIGH", page_index=listener_data["page_index"], bot_instance=self)
                        await listener_data["message"].edit(view=ephemeral_view)
                    except Exception:
                        if listener_data in self.active_thought_listeners.get(placeholder_msg.id, []):
                            self.active_thought_listeners[placeholder_msg.id].remove(listener_data)
        ...
        final_thoughts_sanitized = sanitize_thoughts(final_thoughts_str) if final_thoughts_str else "Visualizing layout..."
        if placeholder_msg.id in self.active_thought_listeners:
            for listener_data in self.active_thought_listeners[placeholder_msg.id]:
                try:
                    completed_view = EphemeralThoughtsView(final_thoughts_sanitized, thinking_active=False, current_level="HIGH", page_index=listener_data["page_index"], bot_instance=self)
                    await listener_data["message"].edit(view=completed_view)
                except Exception: pass
    ...
    async def _generate_decoupled_layout(self, channel, author, dsl_instructions: str, placeholder_msg: discord.Message, context_history: str, initial_prompt: str, config_state: dict, user_app_session_id: Optional[int] = None, banter_text: str = ""):
        """Asynchronous decoupled task coordinating the background compilation turn, retries, and handover."""
        logger.info(f"Decoupled Layout compile background job triggered. Instructions: '{dsl_instructions[:30]}...'")
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ai_code, final_thoughts, elapsed_total = await self._stream_layout_and_get_params(
                    raw_prompt=compiler_prompt, context_history=context_history, 
                    channel=channel, placeholder_msg=placeholder_msg
                )
                ...
                if compile_error:
                    logger.warning(f"Decoupled layout compile pass {attempt} failed: {compile_error}. Retrying...")
                    if attempt < max_attempts:
                        ...
                        raise ValueError(f"Max compilation retries exceeded. Final error: {compile_error}")
                
                banter_view = DynamicView(self, channel, user_app_session_id=user_app_session_id)
                if final_thoughts:
                    banter_view.add_item(ThoughtsButton(final_thoughts, elapsed=elapsed_total, thinking_active=False, message_id=placeholder_msg.id, bot_instance=self, thinking_level="HIGH"))
                ...
                logger.error(f"Decoupled Layout background compile attempt {attempt} failed: {exc}")
                
                if attempt == max_attempts:
                    logger.error("Layout compilation failed after all retries. Silently removing broken placeholder.")
                    try:
    ...
    async def _stream_artist_and_get_params(self, raw_prompt, context_history, channel, placeholder_msg, base_image_bytes=None, is_edit_flow=False):
        ...
        current_banter = placeholder_msg.content.replace("*(🎨 Generating Image...)*", "").replace("*(🎨 Generating next version...)*", "").strip() if placeholder_msg.content else ""
        text_content = current_banter + "\n\n*(🎨 Priesty is drafting composition...)*"
        
        view = DynamicView(self, channel)
        btn = ThoughtsButton("Drafting canvas...", elapsed=0, thinking_active=True, message_id=placeholder_msg.id, bot_instance=self, thinking_level="HIGH")
        view.add_item(btn)
        view.finalize_layout()
        await placeholder_msg.edit(content=text_content, view=view)

        try:
            stream = await self.chat_handler.generate_artist_stream(raw_prompt, context_history, base_image_bytes)
        except Exception as e:
            logger.warning(f"Premium artist planning engine failed ({e}). Re-routing immediately to fallback model...")
        ...
        async for chunk in stream:
            ...
            if now - last_edit >= 1.8:
                ...
                sanitized_payload = sanitize_thoughts(current_thoughts_str) if current_thoughts_str else "Visualizing..."
        ...
        if placeholder_msg.id in self.active_thought_listeners:
            for listener_data in self.active_thought_listeners[placeholder_msg.id]:
                try:
                    completed_view = EphemeralThoughtsView(final_thoughts_str, thinking_active=False, current_level="HIGH", page_index=listener_data["page_index"], bot_instance=self)
                    await listener_data["message"].edit(view=completed_view)
                except Exception: 
                    pass
            del self.active_thought_listeners[placeholder_msg.id]
        
        json_match = re.search(r'```json\s*(.*?)\s*```', final_text_str, flags=re.DOTALL | re.IGNORECASE)
        data = {}
        if json_match:
            try:
        ...
        clean_summary = re.sub(r'(?i)<\s*thought\b[^>]*>?\s*.*?(?:<\s*/\s*thought\s*>|$)', '', final_text_str, flags=re.DOTALL)
        clean_summary = re.sub(r'```json.*?```', '', clean_summary, flags=re.DOTALL | re.IGNORECASE).strip()
        
        return data, clean_summary, final_thoughts_str, elapsed_total

    async def _generate_decoupled_image(self, channel, author, raw_image_prompt: str, placeholder_msg: discord.Message, context_history: str, is_edit_flow: bool = False, original_message: discord.Message = None, banter: str = "", disabled_triggers: dict = None, config: dict = None):
        logger.info(f"Artist decoupled background generation triggered. Edit Mode: {is_edit_flow} | Input: '{raw_image_prompt}'")
                ...
                for att in original_message.attachments:
                    if att.content_type and att.content_type.startswith("image/"):
                        try:
                            ...
                            logger.error(f"Failed to parse source attachments from message: {read_err}")
            
            if not base_image_bytes and original_message and original_message.reference and original_message.reference.message_id:
        ...
        try:
            past_creations = await memory.fetch_recent_visual_memories(self, self.brain_server_id, author, limit=3)
            if past_creations:
                ledger_notes = ["\n\n=== PAST VISUAL STYLING REFERENCE ==="]
                for p_idx, item in enumerate(past_creations):
                    ledger_notes.append(f"- Project {p_idx+1}: Prompt=\"{item['prompt']}\" | Preset=\"{item['style']}\" | Ratio=\"{item['ratio']}\" | Seed={item['seed']}")
                context_history += "\n".join(ledger_notes) + "\n==================================\n"
        except Exception as fetch_err:
            logger.warning(f"Failed to fetch user's visual history ledger: {fetch_err}")

        try:
            data, clean_summary, final_thoughts, elapsed_total = await self._stream_artist_and_get_params(
                raw_image_prompt, context_history, channel, placeholder_msg, base_image_bytes, is_edit_flow
            )
        except Exception as planning_exc:
            logger.error(f"Single-pass LLM prompt expansion pass failed: {planning_exc}")
            data = {
        ...
        try:
            ...
            try:
                stream = await self.chat_handler.generate_reply_stream(
                    message_content=complain_prompt, channel_history=context_history,
                    attachments=[], user_display_name=author.display_name, user_memory={},
                    server_context="", thinking_level="MINIMAL"
                )
                complaint_text = ""
                async for chunk in stream:
                    if chunk.candidates and chunk.candidates[0].content and chunk.candidates[0].content.parts:
                        chunk_text = "".join(p.text for p in chunk.candidates[0].content.parts if getattr(p, 'text', None) and not getattr(p, 'thought', False))
            ...
            view = DynamicView(self, channel, disabled_triggers=disabled_triggers, original_message=original_message, user_app_session_id=user_app_session_id)
            view.add_image_controls(
                self, placeholder_msg.id, current_prompt=raw_image_prompt, current_style=selected_style, 
                current_ratio=selected_ratio, current_strength=selected_strength, current_is_edit_flow=is_edit_flow, show_actions=True
            )
            view.add_item(ThoughtsButton(final_thoughts, elapsed=elapsed_total, thinking_active=False, message_id=placeholder_msg.id, bot_instance=self, thinking_level="HIGH"))
            view.finalize_layout()
        ...
        clean_banter = original_banter.replace("*(🎨 Generating Image...)*", "").replace("*(🎨 The Artist is drafting composition...)*", "").replace("*(🎨 Image Generated)*", "").strip()
        if clean_banter == "🎨 *Generating Image...*": 
            clean_banter = ""
        
        if clean_summary:
            clean_banter = clean_banter + ("\n\n" if clean_banter else "") + clean_summary
        ...
        version_payload = {
            "prompt": raw_image_prompt, "expanded": expanded_prompt, "style": selected_style, "ratio": selected_ratio,
            "strength": selected_strength, "seed": seed, "image_bytes": img_bytes, "is_completed": True,
            "banter": clean_banter, "is_edit_flow": is_edit_flow, "thoughts": final_thoughts, "thoughts_elapsed": elapsed_total,
            "user_app_session_id": user_app_session_id
        }
        self.image_versions[placeholder_msg.id].append(version_payload)
        self.image_version_indexes[placeholder_msg.id] = len(self.image_versions[placeholder_msg.id]) - 1
        
        try:
            cdn_url = ""
            await self._update_image_message_view(placeholder_msg, channel, version_payload, user_app_session_id=user_app_session_id)
            if placeholder_msg.attachments:
                cdn_url = placeholder_msg.attachments[0].url
                
            await memory.save_visual_memory(
                self, self.brain_server_id, author, raw_image_prompt, selected_style, selected_ratio, seed, cdn_url
            )
        except Exception as save_err:
            logger.warning(f"Failed to write visual metadata to user's database: {save_err}")

    async def process_image_generation_update(self, interaction: discord.Interaction, message_id: int, prompt: str, style: str, ratio: str, strength: str = "0.6", generation_mode: str = "t2i", seed: int = None, regenerate: bool = False):
        msg = interaction.message if interaction.message else await interaction.channel.fetch_message(message_id)
        
        prev_banter = ""
        versions = self.image_versions.get(message_id, [])
        user_app_session_id = versions[-1].get("user_app_session_id") if versions else None
        
        if versions: prev_banter = versions[-1].get("banter", "")
        ...
        if loading_indicator not in current_text:
            current_text = current_text.replace("*(🎨 Image Generated)*", "").strip() + loading_indicator
        ...
        temp_view = DynamicView(self, interaction.channel, user_app_session_id=user_app_session_id)
        temp_view.add_image_controls(self, message_id, prompt, style, ratio, strength, current_is_edit_flow=is_edit_flow, show_actions=False)
        ...
        self.loop.create_task(
            self._execute_image_generation_update_task(
                message_id=message_id, prompt=prompt, style=style, ratio=ratio, strength=strength, seed=seed,
                base_image_bytes=base_image_bytes, is_edit_flow=is_edit_flow, prev_banter=prev_banter, msg_target=msg, channel=interaction.channel,
                user_app_session_id=user_app_session_id, author=interaction.user
            )
        )

    async def _execute_image_generation_update_task(self, message_id: int, prompt: str, style: str, ratio: str, strength: str, seed: int, base_image_bytes: bytes, is_edit_flow: bool, prev_banter: str, msg_target: discord.Message, channel, user_app_session_id=None, author=None):
        logger.info(f"Artist parameters update background generation task triggered. Style: {style} | Ratio: {ratio}")
        ...
        try:
            ...
            clean_summary = ""
            final_thoughts = "Direct prompt update fallback. Planning engine was bypassed."
            elapsed_total = 0
        ...
        try:
            img_bytes = await self.image_generator.generate(
                prompt=expanded_prompt, width=width, height=height, seed=seed, style_key=style, 
                base_image_bytes=base_image_bytes, strength=strength
            )
        except image_gen.SafetyBlockError as safe_err:
            logger.info("Safety block triggered during edit update. Generating complaint.")
            ...
            try:
                stream = await self.chat_handler.generate_reply_stream(
                    message_content=complain_prompt, channel_history=self.history_tracker.get_formatted_history(channel.id),
                    attachments=[], user_display_name="User", user_memory={}, server_context="", thinking_level="MINIMAL"
                )
                complaint_text = ""
                async for chunk in stream:
                    if chunk.candidates and chunk.candidates[0].content and chunk.candidates[0].content.parts:
            ...
            view.add_image_controls(self, msg_target.id, current_prompt=prompt, current_style=style, current_ratio=ratio, current_strength=strength, current_is_edit_flow=is_edit_flow, show_actions=True)
            view.finalize_layout()
            complaint_text = await self._resolve_mentions(complaint_text, channel)
            ...
            logger.error(f"Unified fallback update task was completely exhausted: {final_fail_exc}")
            try:
                versions = self.image_versions.get(message_id, [])
                if versions:
                    last_version = versions[-1]
                    await self._update_image_message_view(msg_target, channel, last_version, user_app_session_id=user_app_session_id)
                    content_note = (prev_banter or "").replace("*(🎨 Generating next version...)*", "").strip()
                    ...
                    clean_content = clean_content.replace("*(🎨 Generating next version...)*", "").strip()
                    error_text = clean_content + f"\n\n*(❌ Parameter update failed: {final_fail_exc})*"
                    view = DynamicView(self, channel, user_app_session_id=user_app_session_id)
                    view.finalize_layout()
                    error_text = await self._resolve_mentions(error_text, channel)
                ...
                logger.error(f"Failed to cleanly recover and render error fallback: {recover_err}")
            return
        ...
        version_payload = {
            "prompt": prompt, "expanded": expanded_prompt, "style": style, "ratio": ratio,
            "strength": strength, "seed": seed, "image_bytes": img_bytes, "is_completed": True,
            "banter": final_banter, "is_edit_flow": is_edit_flow, "thoughts": final_thoughts, "thoughts_elapsed": elapsed_total,
            "user_app_session_id": user_app_session_id
        }
        self.image_versions[message_id].append(version_payload)
        self.image_version_indexes[message_id] = len(self.image_versions[message_id]) - 1

        try:
            cdn_url = ""
            await self._update_image_message_view(msg_target, channel, version_payload, user_app_session_id=user_app_session_id)
    ...
    async def _update_image_message_view(self, message: discord.Message, channel, version_payload: dict, user_app_session_id=None):
        ...
        clean_content = clean_content.replace("*(🎨 Generating Image...)*", "").replace("*(🎨 Generating next version...)*", "").replace("*(🎨 The Artist is drafting composition...)*", "").replace("*(🎨 Image Generated)*", "").strip()
        
        if is_completed: clean_content += "\n\n*(🎨 Image Generated)*"
        if not clean_content or clean_content == "🎨 *Generating Image...*": clean_content = f"🎨 Here is your drawing of \"{prompt[:80]}\"!\n\n*(🎨 Image Generated)*"
            
        view = DynamicView(self, channel, user_app_session_id=user_app_session_id)
        if final_thoughts: view.add_item(ThoughtsButton(final_thoughts, elapsed=thoughts_elapsed, thinking_active=False, message_id=message.id, bot_instance=self, thinking_level="HIGH"))
        view.add_image_controls(self, message.id, prompt, style, ratio, strength, current_is_edit_flow=is_edit_flow)
        view.finalize_layout()
        
        clean_content = await self._resolve_mentions(clean_content, channel)
        await message.edit(content=clean_content, embed=None, view=view, attachments=[file])
    ...
    async def _apply_typo_edit(self, message: discord.Message, typo_edit_data: tuple[str, str]):
        ...
        except Exception as e: logger.warning(f"Failed to apply typo self-edit correction: {e}")

    def _compile_server_context(self, guild: discord.Guild, member: discord.Member = None) -> str:
        if not guild: return "Environment: Direct Messages."
        
        context = f"Current Server: {guild.name}\nAvailable Text Channels to Mention:\n"
        for channel in guild.text_channels: context += f"- #{channel.name}: Use mention tag <#{channel.id}>\n"
            
        if member and isinstance(member, discord.Member) and member.voice and member.voice.channel:
        ...
        if guild.emojis:
            for emoji in guild.emojis:
                syntax = f"<a:{emoji.name}:{emoji.id}>" if emoji.animated else f"<:{emoji.name}:{emoji.id}>"
                context += f"- Name: :{emoji.name}: | Syntax: {syntax}\n"
        else: context += "- No custom emojis available in this server.\n"
        ...
        while not self.is_closed():
            ...
            try:
                for guild in self.guilds:
                    ...
                    import core.memory as memory
                    state = await memory.load_news_state(self, self.brain_server_id, guild.id)
                    if not state:
                        state = {
                            "last_episode_number": 0,
                            "show_name": "",
                    ...
                    if (hour == 8 and minute >= 30) and state.get("last_morning_pregen_date") != date_str:
                        state["last_morning_pregen_date"] = date_str
                        await memory.save_news_state(self, self.brain_server_id, guild.id, state)
                        self.loop.create_task(self._run_pregen_pipeline(guild.id, "morning", config, state))
                        
                    elif (hour >= 9 and hour < 12) and state.get("last_morning_broadcast_date") != date_str:
                        if state.get("last_morning_pregen_date") == date_str:
                            state["last_morning_broadcast_date"] = date_str
                            await memory.save_news_state(self, self.brain_server_id, guild.id, state)
                            self.loop.create_task(self._run_broadcast_pipeline(guild.id, "morning", config, state))

                    elif (hour == 19 and minute >= 30) and state.get("last_night_pregen_date") != date_str:
                        state["last_night_pregen_date"] = date_str
                        await memory.save_news_state(self, self.brain_server_id, guild.id, state)
                        self.loop.create_task(self._run_pregen_pipeline(guild.id, "night", config, state))

                    elif (hour >= 20 or hour < 4) and state.get("last_night_broadcast_date") != date_str:
                        if state.get("last_night_pregen_date") == date_str:
                            state["last_night_broadcast_date"] = date_str
                            await memory.save_news_state(self, self.brain_server_id, guild.id, state)
                            self.loop.create_task(self._run_broadcast_pipeline(guild.id, "night", config, state))
                            
            except Exception as e:
                logger.error(f"Error inside automated news cron loop: {e}", exc_info=True)

    async def _generate_show_branding(self, server_name: str, gemini_key: str, news_model: str) -> str:
        ...
        try:
            ...
            if response and response.text:
                ...
                return show_name if len(show_name) >= 4 else f"{server_name} Daily Chronicle"
        except Exception as e:
            logger.warning(f"Branding generation failed, falling back to default: {e}")
        return f"{server_name} Daily Chronicle"

    async def _run_pregen_pipeline(self, guild_id: int, edition: str, config: dict, state: dict):
        """Asynchronous, non-blocking scheduler controls resources and offloads rendering processes."""
        logger.info(f"[Server News: {edition.upper()}] Initiating pre-generation pipeline for Guild ID {guild_id}...")
        ...
        if not show_name:
            show_name = await self._generate_show_branding(server_name, gemini_key, news_model)
            state["show_name"] = show_name
            import core.memory as memory
            await memory.save_news_state(self, self.brain_server_id, guild_id, state)
        ...
        show_length = config.get(f"{edition}_length", "Standard")
        from tools.news.script_writer import write_news_script
        try:
            segments = await write_news_script(
                edition=edition, 
            ...
            logger.error(f"[Server News: {edition.upper()}] Script writing failed: {script_err}", exc_info=True)
            return

        self.loop.create_task(
            self._execute_threaded_rendering(guild_id, edition, episode, show_name, segments, state)
        )

    async def _execute_threaded_rendering(self, guild_id: int, edition: str, episode: int, show_name: str, segments: list, state: dict):
        """Offloads rendering processes and uploads outputs to Streamable on worker threads."""
        logger.info(f"[Server News: {edition.upper()}] Starting non-blocking render thread pool...")
        music_path = "assets/late_night_jazz.mp3" if edition == "night" else "assets/morning_acoustic.mp3"
        local_output_filename = f"temp_{edition}_edition_broadcast_{guild_id}.mp4"
        ...
        try:
            ...
            logger.error(f"[Server News: {edition.upper()}] Video compiling thread crashed: {render_err}", exc_info=True)
            return
        ...
        try:
            ...
            logger.error(f"[Server News: {edition.upper()}] Video uploading thread crashed: {upload_err}", exc_info=True)
            return

        if not streamable_url:
            logger.warning(f"[Server News: {edition.upper()}] Pre-generation uploaded null url.")
            return
        ...
        import core.memory as memory
        await memory.save_news_state(self, self.brain_server_id, guild_id, state)
        logger.info(f"[Server News: {edition.upper()}] Success! Staged video link cached: {streamable_url}")

    async def _run_broadcast_pipeline(self, guild_id: int, edition: str, config: dict, state: dict):
        """Asynchronous airing pipeline that retrieves staged links and broadcasts announcements."""
        logger.info(f"[Server News: {edition.upper()}] Initiating Airing Broadcast pipeline for Guild ID {guild_id}...")
        
        broadcast_channel_id = config.get("news_channel_id")
        if not broadcast_channel_id:
            logger.warning(f"[Server News: {edition.upper()}] Broadcast aborted: No target News channel is defined.")
            return
            
        channel = self.get_channel(broadcast_channel_id)
        if not channel:
            try:
                channel = await self.fetch_channel(broadcast_channel_id)
            except Exception:
                logger.warning(f"[Server News: {edition.upper()}] Broadcast aborted: Target News channel ID {broadcast_channel_id} is unreachable.")
                return

        staged_url = state.get(f"staged_{edition}_url")
        episode = state.get(f"staged_{edition}_episode", 1)
        show_name = state.get("show_name", "Conquest News")
        
        if not staged_url:
            logger.error(f"[Server News: {edition.upper()}] Airing failed: No pre-generated staged Streamable URL was found.")
            return
        ...
        logger.info(f"[Server News: {edition.upper()}] Generating daily plain-text summary via Gemini...")
        summary_text = ""
        try:
            ...
            logger.warning(f"Summary generation failed, using standard placeholder: {e}")
            summary_text = (
        ...
        final_announcement = await self._resolve_mentions(final_announcement, channel)

        try:
            await channel.send(content=final_announcement)
            logger.info(f"[Server News: {edition.upper()}] Success! Daily Broadcast Announcement sent to {channel.name}.")
        except Exception as post_err:
            logger.error(f"[Server News: {edition.upper()}] Failed to post broadcast message: {post_err}", exc_info=True)
            return
        ...
        if os.path.exists(local_output_filename):
            try:
                ...
                logger.info(f"[Server News: {edition.upper()}] Cleared local staging cache file: {local_output_filename}")
            except Exception as rm_err:
                logger.warning(f"Could not clear local staging cache file: {rm_err}")

    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        """
        Auto-Leave Event: Disconnects the session when the voice channel becomes empty.
        Only triggers when a non-bot user leaves the bot's active channel.
        """
        if member.bot:
            return
            
        if member.guild.id in self.voice_sessions:
            session = self.voice_sessions[member.guild.id]
            vc_channel = session.vc.channel
            
            if before.channel and before.channel.id == vc_channel.id:
                if after.channel is None or after.channel.id != vc_channel.id:
                    
                    active_users = [m for m in vc_channel.members if not m.bot]
                    
                    if len(active_users) == 0:
                        logger.info(f"No active members remain in voice channel {vc_channel.name}. Leaving automatically.")
                        try:
                            await session.stop()
                        except Exception as e:
                            logger.warning(f"Error disconnecting from empty voice channel: {e}")


    async def on_socket_raw_receive(self, msg):
        """Intercepts raw gateway packets and forwards voice handshakes to our proxy."""
        if not msg:
            return
        try:
            import json
            ...
            if t in ("VOICE_STATE_UPDATE", "VOICE_SERVER_UPDATE"):
                guild_id = packet["d"].get("guild_id")
                if guild_id:
                    session = self.voice_sessions.get(int(guild_id))
                    if session:
                        await session.handle_gateway_packet(t, packet["d"])
        except Exception:
            pass