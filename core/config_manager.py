...
class ConfigManager:
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS server_configs (
                    ...
                    entity_disabled_tools_json TEXT DEFAULT '{}',
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                ON CONFLICT(guild_id) DO UPDATE SET
                    ...
                    entity_disabled_tools_json = excluded.entity_disabled_tools_json,
    ...
    def set_server_entity_disabled_tools(self, guild_id: str | int, entity_ids: list[str], disabled_tools: list[str]):
        current = self.get_server_config(guild_id)
        edt_map = current.get("entity_disabled_tools", {})
        for eid in entity_ids:
            clean_eid = str(eid).strip()
            if clean_eid:
                edt_map[clean_eid] = disabled_tools
        self.set_server_config(guild_id, entity_disabled_tools=edt_map)
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                INSERT INTO channel_configs (
                    ...
                    disabled_tools_json, entity_disabled_tools_json, memory_policy,
    ...
    def set_channel_entity_disabled_tools(self, channel_id: str | int, entity_ids: list[str], disabled_tools: list[str], guild_id: str | int | None = None):
        current = self.get_channel_config(channel_id)
        edt_map = current.get("entity_disabled_tools", {})
        for eid in entity_ids:
            clean_eid = str(eid).strip()
            if clean_eid:
                edt_map[clean_eid] = disabled_tools
        self.set_channel_config(channel_id, guild_id=guild_id, entity_disabled_tools=edt_map)
    ...
    def resolve_effective_config(
        ...
        if user_id:
            uid_str = str(user_id)
            s_entity_dt = s_cfg.get("entity_disabled_tools", {})
            if uid_str in s_entity_dt:
                disabled_tools.update(s_entity_dt[uid_str])

            c_entity_dt = c_cfg.get("entity_disabled_tools", {})
            if uid_str in c_entity_dt:
                disabled_tools.update(c_entity_dt[uid_str])
