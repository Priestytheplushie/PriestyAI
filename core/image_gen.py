...
class ImageGenerator:
    ...
    async def generate(self, prompt: str, width: int = 1024, height: int = 1024, seed: int = None, style_key: str = None, base_image_bytes: bytes = None, strength: str = "0.6") -> bytes:
        ...
        try:
            ...
            img_bytes = await self._generate_pollinations(styled_prompt, width, height, seed)
            if img_bytes:
                logger.info("Success: Generation completed via Pollinations (Attempt 1).")
                return img_bytes
        ...
        if self.pixazo_api_key:
            try:
                logger.info("Attempting Image Generation [Attempt 2/4]: Pixazo API Gateway...")
                img_bytes = await self._generate_pixazo(styled_prompt, width, height, seed, style_key)
                if img_bytes:
            ...
            logger.warning("Pixazo Attempt skipped: PIXAZO_API_KEY / IMG_API_KEY environment variable is not defined.")
            errors.append("Pixazo Attempt: API Key missing")

        try:
            logger.info("Attempting Image Generation [Attempt 3/4]: Pollinations AI (Retry)...")
            img_bytes = await self._generate_pollinations(styled_prompt, width, height, seed)
            if img_bytes:
                logger.info("Success: Generation completed via Pollinations (Attempt 3).")
                return img_bytes
        except Exception as e:
        ...
        try:
            logger.info("Attempting Image Generation [Attempt 4/4]: AI Horde (Asynchronous Polling)...")
        ...
        raise ValueError("Image generation failed across all fallback backends: " + " | ".join(errors))


    async def _generate_pollinations(self, styled_prompt: str, width: int, height: int, seed: int) -> bytes:
        now = asyncio.get_event_loop().time()
        if now < self.pollinations_cooldown_until:
            remaining = int(self.pollinations_cooldown_until - now)
            logger.info(f"Skipping Pollinations: Active cooldown is in place ({remaining}s remaining).")
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.get(request_url, timeout=30, ssl=False) as response:
                ...
                if response.status in (402, 429):
                    self.pollinations_cooldown_until = asyncio.get_event_loop().time() + 300.0
                    logger.warning(f"Pollinations returned HTTP {response.status}. Triggering 5-minute lockout timer.")
                    
                raise ValueError(f"HTTP Status {response.status}")

    async def _generate_pixazo(self, styled_prompt: str, width: int, height: int, seed: int, style_key: str) -> bytes:
        headers = {
        ...
        connector = aiohttp.TCPConnector(ssl=False)
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.post(url, json=payload, headers=headers, timeout=40, ssl=False) as response:
                if response.status != 200:
                ...
                if not img_url:
                    raise ValueError(f"Invalid API response payload structure: {resp_json}")
                
                async with session.get(img_url, timeout=30, ssl=False) as img_resp:
                    if img_resp.status == 200:
                        return await img_resp.read()
                    raise ValueError(f"Failed to download image asset from {img_url}. Status: {img_resp.status}")

    async def _generate_aihorde(self, styled_prompt: str, width: int, height: int, seed: int) -> bytes:
        headers = {
            "Content-Type": "application/json",
            "apikey": self.aihorde_key
        }
        ...
        connector = aiohttp.TCPConnector(ssl=False)
        async with aiohttp.ClientSession(connector=connector) as session:
            async with session.post(self.aihorde_post_url, json=payload, headers=headers, timeout=25, ssl=False) as response:
                if response.status != 202:
                    text_err = await response.text()
                    raise ValueError(f"Job dispatch failed with HTTP Status {response.status}: {text_err}")
                resp_json = await response.json()
                job_id = resp_json.get("id")
                if not job_id:
                    raise ValueError(f"Missing job tracking identifier from AI Horde: {resp_json}")

            completed_url = None
            for check_idx in range(16):
                await asyncio.sleep(2.5)
                check_endpoint = f"{self.aihorde_check_url}{job_id}"
                async with session.get(check_endpoint, timeout=15, ssl=False) as check_resp:
                    if check_resp.status == 200:
                        ...
                        logger.warning(f"AI Horde polling state warning on check {check_idx+1}: HTTP {check_resp.status}")

            if not completed_url:
                raise TimeoutError("AI Horde queues timed out waiting for a volunteer GPU to claim execution.")

            async with session.get(completed_url, timeout=15, ssl=False) as status_resp:
                if status_resp.status != 200:
                    raise ValueError(f"Failed to fetch completed status from AI Horde: HTTP {status_resp.status}")
                final_json = await status_resp.json()
                generations = final_json.get("generations")
                if not generations or not isinstance(generations, list) or len(generations) == 0:
                    raise ValueError(f"AI Horde returned invalid generation data structures: {final_json}")
                
                download_url = generations[0].get("img")
                if not download_url:
                    raise ValueError(f"AI Horde status returned empty image link: {generations[0]}")

            async with session.get(download_url, timeout=20, ssl=False) as download_resp:
                if download_resp.status == 200:
                    return await download_resp.read()
                raise ValueError(f"Failed to download image payload from AI Horde CDN {download_url}. Status: {download_resp.status}")