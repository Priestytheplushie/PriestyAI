...
from agents.discord_react.views import (
    AgentContinuationView,
    AgentErrorView,
    AgentStepButton,
)
...
class AgentSession(BaseAgentSession):
    """
    Standard ReAct Loop Agent Session.
    Maintains autonomous step state, error retry patterns, and tool dispatch.
    """
    ...
    def compile_react_transcript(self) -> str:
        """Converts the active step logs into a clean historical timeline context."""
        ...
        for i, step in enumerate(self.react_history):
            ...
            lines.append(
                f"Action Call: `{step.get('tool', '')}` with args {json.dumps(step.get('args', {}))}"
            )
            lines.append(f"Observation Result: {step.get('observation', '')}\n")
        return "\n".join(lines)

    async def execute_tick(self, bot):
        """Asynchronous execution controller tick driving the ReAct model forward."""
        if self.status != "running":
            logger.info(
                f"Session {self.thread_id} tick bypassed: Status is '{self.status}'"
            )
            return
        ...
        step_btn = AgentStepButton(
            step_index=self.current_step - 1, session_id=self.thread_id
        )
        step_btn.disabled = True
        progress_view.add_item(step_btn)

        progress_msg = await self.channel.send(
            content=progress_content, view=progress_view
        )
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Could not locate agent prompt markdown instructions file at {PROMPT_PATH}"
                )

            contexts_header = (
                self.loaded_contexts
                if self.loaded_contexts
                else "No contexts attached."
            )
            ...
            logger.info(
                f"Invoking LLM step {self.current_step} inside thread {self.thread_id}"
            )
            ...
            for attempt in range(max_retries):
                try:
                    response = (
                        await bot.chat_handler.client.aio.models.generate_content(
                    ...
                    if "MALFORMED_RESPONSE" in finish_reason_str:
                        logger.warning(
                            "Agent Loop detected MALFORMED_RESPONSE. Falling back to standard generation..."
                        )
                        response = (
                            await bot.chat_handler.client.aio.models.generate_content(
                                model=bot.chat_handler.premium_model,
                    ...
                    logger.warning(
                        f"Agent step loop API attempt {attempt + 1} failed: {error_str}"
                    )
            ...
            thought, tool, args, parse_error = self.parse_react_output(
                output_text, native_thoughts
            )

            if (
                not tool or tool.lower().strip() in ("none", "final_report")
            ) and self.current_step == 1:
                logger.warning(
                    "Agent returned blank/none action on Step 1. Retrying with a standard recovery pass..."
                )
                try:
                    response = (
                        await bot.chat_handler.client.aio.models.generate_content(
            ...
            if parse_error:
                ...
                await progress_msg.edit(
                    content=f"⚠️ **Step {self.current_step}: Formatting validation failed**"
                )
            ...
            if (
                not tool
                or tool.lower().strip() == "none"
                or tool.lower().strip() == "final_report"
            ):
                logger.info(
                    f"Agent session {self.thread_id} successfully finalized its goals."
                )
                self.status = "completed"
                step_btn.disabled = False
                await progress_msg.edit(
                    content=f"✅ **Step {self.current_step}: Completed analysis!**",
                    view=progress_view,
                )
            ...
            if self.status == "running":
                step_btn.disabled = False
                await progress_msg.edit(
                    content=f"✅ **Step {self.current_step}: Completed task `{tool}`**",
                    view=progress_view,
                )
                bot.loop.create_task(self.execute_tick(bot))
            ...
            logger.error(
                f"Unstable backend server error during agent turn {self.current_step}: {s_err}"
            )
            self.status = "paused_error"
            await progress_msg.edit(
                content=f"❌ **Step {self.current_step}: Terminated due to API server error**"
            )
            ...
            self.status = "paused_error"
            await progress_msg.edit(
                content=f"❌ **Step {self.current_step}: Investigation crashed**"
            )
    ...
    def parse_react_output(
        self, output: str, native_thoughts: str = ""
    ) -> tuple[str, Optional[str], dict, Optional[str]]:
        """Parses LLM thoughts and structured tool JSON strings."""
        thought_match = re.search(
            r"(?i)```thought\s*(.*?)\s*```", output, flags=re.DOTALL
        )
        if not thought_match:
            thought_match = re.search(
                r"(?i)<\s*thought\s*>\s*(.*?)\s*<\s*/\s*thought\s*>",
                output,
                flags=re.DOTALL,
            )

        action_match = re.search(
            r"(?i)```action\s*(.*?)\s*```", output, flags=re.DOTALL
        )
        if not action_match:
            action_match = re.search(
                r"(?i)<\s*action\s*>\s*(.*?)\s*<\s*/\s*action\s*>",
                output,
                flags=re.DOTALL,
            )
        ...
        try:
            ...
            return (
                ...
                None,
                {},
                f"Invalid Action block JSON syntax: {json_err}",
            )
        ...
        if not tool_name:
            return (
                ...
                None,
                {},
                "Missing 'tool' property inside Action block JSON.",
            )

        return thought_content, tool_name, arguments, None

    async def finalize_report(self, bot, raw_model_output: str = "") -> None:
        """Assembles the final comprehensive summary post, extracting raw summaries or compiling last turns."""
        final_summary = raw_model_output

        final_summary = re.sub(
            r"(?i)```thought\s*.*?\s*```", "", final_summary, flags=re.DOTALL
        )
        final_summary = re.sub(
            r"(?i)<\s*thought\s*>.*?</\s*thought\s*>",
            "",
            final_summary,
            flags=re.DOTALL,
        )
        final_summary = re.sub(
            r"(?i)```action\s*.*?\s*```", "", final_summary, flags=re.DOTALL
        )
        final_summary = re.sub(
            r"(?i)<\s*action\s*>.*?</\s*action\s*>", "", final_summary, flags=re.DOTALL
        )
        ...
        await self.channel.send(
            content="💡 *The private workspace has successfully concluded its operations.*"
        )
