...
async def run_local_agent_tool(bot, session, tool_name: str, arguments: dict) -> str:
    """Dispatches tool call payload requests to secure Python coroutines."""
    ...
    try:
        if t_name == "read_channel_history":
            ...
            return await tool_read_channel_history(
                bot, session, channel_id, limit, before_id
            )
            ...
            return await tool_search_server_messages(
                bot, session, query, author_id, limit
            )
            ...
            question = arguments.get(
                "question_text", "I require your assistance to proceed."
            )
            component_type = arguments.get("component_type", "Button").strip()
            options = arguments.get("suggested_options", [])
            return await tool_ask_user_question(
                bot, session, question, component_type, options
            )
            ...
            return await tool_compare_user_activity(
                bot, session, user_id_1, user_id_2, timeframe_days
            )
            ...
            return await tool_read_message_attachment(
                bot, session, channel_id, message_id, attachment_url
            )
            ...
            return await tool_update_context_snapshot(
                bot, session, alias, updated_data, notes
            )
...
async def tool_read_channel_history(
    bot, session, channel_id: int, limit: int, before_msg_id: Optional[int]
) -> str:
    ...
    user_member = guild.get_member(session.user_id) or await guild.fetch_member(
        session.user_id
    )
    if not user_member:
    ...
    try:
        ...
        async for msg in channel.history(limit=limit, before=before_msg):
            ...
            lines.append(
                f"[{timestamp}] MessageID: {msg.id} | {author_name}: {msg.clean_content}"
            )
...
async def tool_search_server_messages(
    bot, session, query: str, author_id: Optional[int], limit: int
) -> str:
    guild = session.channel.guild
    if hasattr(session, "target_guild_id") and session.target_guild_id:
        guild = bot.get_guild(session.target_guild_id)
        ...
        try:
            ...
            async for msg in channel.history(limit=100):
                ...
                if query.lower() in msg.content.lower():
                    author_name = f"{msg.author.display_name} (@{msg.author.name})"
                    lines.append(
                        f"Channel: #{channel.name} | MessageID: {msg.id} | {author_name}: {msg.clean_content}"
                    )
                    found_count += 1
        except Exception:
            continue
...
async def tool_fetch_user_profile(bot, session, user_id: int) -> str:
    ...
    try:
        ...
        return (
            f"[Error: User with ID {user_id} is not a member of guild '{guild.name}']"
        )
...
async def tool_ask_user_question(
    bot, session, question: str, component_type: str = "Button", options: list = None
) -> str:
    session.status = "paused_user_question"
    session.pending_question_text = question
    session.pending_options = (
        [o.strip() for o in options if o.strip()][:25] if options else []
    )

    logger.info(
        f"Agent session {session.thread_id} paused to prompt user question: '{question[:20]}...'"
    )
    ...
    if comp_clean == "channelselect":
        select = discord.ui.ChannelSelect(
            ...
            custom_id=f"agent_q_chan_{session.thread_id}",
        )
        ...
        async def chan_callback(interaction: discord.Interaction):
            ...
            summary = (
                f"User selected channel: #{selected.name} [ID: {selected.id}]"
                if selected
                else "User submitted an empty channel selection."
            )
        ...
        select = discord.ui.UserSelect(
            ...
            custom_id=f"agent_q_user_{session.thread_id}",
        )
        ...
        async def user_callback(interaction: discord.Interaction):
            ...
            selected = select.values[0] if select.values else None
            summary = (
                f"User selected member: {selected.display_name} (@{selected.name}) [ID: {selected.id}]"
                if selected
                else "User submitted an empty user selection."
            )
            await resume_with_choice(interaction, summary)
        ...
        select = discord.ui.RoleSelect(
            ...
            custom_id=f"agent_q_role_{session.thread_id}",
        )
        ...
        async def role_callback(interaction: discord.Interaction):
            ...
            selected = select.values[0] if select.values else None
            summary = (
                f"User selected role: {selected.name} [ID: {selected.id}]"
                if selected
                else "User submitted an empty role selection."
            )
            await resume_with_choice(interaction, summary)
        ...
        opts = [
            ...
            for opt in session.pending_options
        ]
        select = discord.ui.Select(
            placeholder="Choose an option...",
            options=opts,
            custom_id=f"agent_q_str_{session.thread_id}",
        )
            ...
            for opt in session.pending_options[:5]:
                ...
                def make_btn_callback(selected_opt=opt):
                    async def btn_callback(interaction: discord.Interaction):
                        await interaction.response.defer()
                        await resume_with_choice(
                            interaction, f"User selected option: '{selected_opt}'"
                        )
    ...
    cancel_btn = discord.ui.Button(
        ...
        custom_id=f"agent_cancel_{session.thread_id}",
    )
    ...
    question_chunks = (
        [question[i : i + 1900] for i in range(0, len(question), 1900)]
        if question
        else [""]
    )
...
async def tool_compare_user_activity(
    bot, session, user_id_1: int, user_id_2: int, timeframe_days: int
) -> str:
    guild = session.channel.guild
    if hasattr(session, "target_guild_id") and session.target_guild_id:
        guild = bot.get_guild(session.target_guild_id)
    ...
    profile1 = {
        ...
        "joined_at": (
            member1.joined_at.strftime("%Y-%m-%d") if member1.joined_at else "Unknown"
        ),
    ...
    profile2 = {
        ...
        "joined_at": (
            member2.joined_at.strftime("%Y-%m-%d") if member2.joined_at else "Unknown"
        ),
    ...
    try:
        ...
        for thread in guild.threads:
            ...
            if parent_channel:
                ...
                lines.append(
                    f"  - #{thread.name} ({thread_type}) [ID: {thread.id}] under channel #{parent_channel.name}"
                )
...
async def tool_read_message_attachment(
    bot, session, channel_id: int, message_id: int, attachment_url: str
) -> str:
    guild = session.channel.guild
    if hasattr(session, "target_guild_id") and session.target_guild_id:
        guild = bot.get_guild(session.target_guild_id)
        ...
        async with aiohttp.ClientSession(headers=headers) as http_session:
            async with http_session.get(attachment_url, timeout=15) as response:
                ...
                if (
                    ...
                    or attachment_url.lower().endswith(
...
async def tool_update_context_snapshot(
    bot, session, alias: str, updated_data: dict, notes: str
) -> str:
    guild = session.channel.guild
    if hasattr(session, "target_guild_id") and session.target_guild_id:
        guild = bot.get_guild(session.target_guild_id)
    ...
    success = await memory.save_context_snippet(
        bot, bot.brain_server_id, session.user_id, alias, type_name, updated_data, notes
    )
