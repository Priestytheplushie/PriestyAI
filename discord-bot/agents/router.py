...
<<<<<<< HEAD:agents/router.py
    """
    Finds the first '{' and the last '}' inside the raw model output text
    to cleanly extract the core JSON object, ignoring any conversational noise.
    """
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/agents/router.py
    start = text.find("{")
    end = text.rfind("}")
...
def check_static_heuristics(prompt: str) -> Optional[dict]:
    """
    Bypasses the Gemini API entirely if the prompt contains clear keyword signatures,
    reducing latency to <10ms and preventing API 500 error dependencies [3].
    """
    ...
    if any(
        prompt_clean.startswith(t) or f" {t} " in f" {prompt_clean} "
...
<<<<<<< HEAD:agents/router.py
    """
    Analyzes the user's query and classifies it into an agent type.
    Includes instant static bypass heuristics and robust, rate-limit safe retries.
    """
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/agents/router.py
    ...
    if heuristic_match:
        logger.info(
            f"Heuristics hit! Bypassing API classification. Routed directly to: '{heuristic_match['agent']}'"
        )
        return heuristic_match

    logger.info(
        f"Ambiguous query. Falling back to LLM intent classification: '{user_prompt[:30]}...'"
    )
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            return {
                ...
                "plan": data.get(
                    "plan",
                    "Executing standard information gather and compile protocols.",
                ),
            }
    ...
    logger.warning(
        "All classification retries failed. Running safe fallback heuristics..."
    )
