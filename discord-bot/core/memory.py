...
class ChatHistoryTracker:
    ...
    def get_formatted_history(self, channel_id: int) -> str:
        ...
        for msg in self.histories[channel_id]:
            ...
            lines.append(
                f"[{msg['timestamp']}] {msg['display_name']} (@{msg['username']}): {msg['content']}{attachment_note}"
            )

        return "\n".join(lines)


async def get_or_create_forum_channel(
    guild: discord.Guild, name: str
) -> Optional[discord.ForumChannel]:
<<<<<<< HEAD:core/memory.py
    """Finds or creates a Discord Forum Channel inside the active Guild."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    forum = discord.utils.get(guild.channels, name=name, type=discord.ChannelType.forum)
    if not forum:
        try:
            forum = await guild.create_forum(name=name)
            logger.info(
                f"Database Forum Channel '#{name}' created in Brain Server '{guild.name}' (ID: {guild.id})"
            )
...
async def get_or_create_db_thread(
    guild: discord.Guild,
    forum_name: str,
    thread_name: str,
    initial_content: str = "Database Thread",
) -> Optional[discord.Thread]:
<<<<<<< HEAD:core/memory.py
    """
    Resilient helper that resolves or instantiates dedicated thread files inside Forums.
    Handles unarchiving stale database threads automatically.
    """
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
        ...
        try:
            async for arch_thread in forum.archived_threads(limit=100):
                if arch_thread.name == str(thread_name):
                    ...
                    logger.info(
                        f"Unarchived Database Thread '{thread_name}' in forum '#{forum_name}'"
                    )
                    break
        except Exception as e:
            logger.warning(
                f"Error fetching archived threads in forum '#{forum_name}': {e}"
            )

    if not thread:
        try:
            thread_with_msg = await forum.create_thread(
                name=str(thread_name), content=initial_content
            )
            thread = thread_with_msg.thread
            logger.info(
                f"Created fresh Database Thread '{thread_name}' in forum '#{forum_name}'"
            )
        except Exception as e:
            logger.error(
                f"Failed to create Database Thread '{thread_name}' in forum '#{forum_name}': {e}"
            )
            return None
...
def should_preserve_message(message: discord.Message) -> bool:
    """Heuristic helper to ensure images, attachments, or structural JSON vectors are never consolidated."""
    ...
    if content.startswith("[FACT_VEC]") or content.startswith("[VISUAL MEMORY]"):
        return True
    if content.startswith("**Image Upload:") or content.startswith(
        "**AI Generated Image Saved:"
    ):
        return True
    if "http" in content and (
        ".png" in content or ".jpg" in content or "discordapp" in content
    ):
        return True
    return bool(re.search(r"```json", content))


async def consolidate_memories_if_needed(
...
<<<<<<< HEAD:core/memory.py
    """
    Checks memory thread length and consolidates raw historical text entries,
    preserving structured vector JSON database rows.
    """
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    ...
    logger.info(
        f"Triggering structured memory consolidation for thread '{thread_name}' in '#{forum_name}'"
    )
    ...
    try:
        response = await client.chat_handler.client.aio.models.generate_content(
            model=client.chat_handler.premium_model, contents=prompt
        )
        ...
        if not consolidated_text.strip():
            logger.warning(
                "Consolidation prompt returned empty results. Aborting rewrite to prevent loss."
            )
            return
        ...
        logger.info(
            f"Structured memory consolidation completed successfully for thread '{thread_name}'"
        )
...
async def generate_embedding(client: discord.Client, text: str) -> List[float]:
    """Generates a dense semantic vector representation using Gemini's text-embedding-001 model."""
    try:
...
async def save_fact(
    ...
    fact: str,
    category: str = "PROFILE & IDENTITY",
    score: int = 10,
) -> bool:
<<<<<<< HEAD:core/memory.py
    """Generates text embedding, packages parameters as JSON, and writes to user forum as a file attachment."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    guild = client.get_guild(brain_server_id)
    if not guild:
    ...
    file = discord.File(fp=io.BytesIO(payload_bytes), filename="fact_vec.json")
    await thread.send(content="[FACT_VEC]", file=file)

    await consolidate_memories_if_needed(
        client, brain_server_id, "user-memories", str(user.id)
    )
    return True
...
<<<<<<< HEAD:core/memory.py
    """Saves metadata about a generated visual creation to assist in maintaining asset consistency."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    guild = client.get_guild(brain_server_id)
    if not guild:
...
async def fetch_recent_visual_memories(
    client: discord.Client,
    brain_server_id: int,
    user: discord.User | discord.Member,
    limit: int = 5,
...
<<<<<<< HEAD:core/memory.py
    """Retrieves previous visual layouts and styling prompts created by the target user."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    guild = client.get_guild(brain_server_id)
    if not guild:
    ...
    forum = discord.utils.get(
        guild.channels, name="user-memories", type=discord.ChannelType.forum
    )
    if not forum:
        return []
...
async def save_image_bytes_fact(
    client: discord.Client,
    brain_server_id: int,
    user: discord.User | discord.Member,
    description: str,
    ...
    msg = await thread.send(
        content=f"**AI Generated Image Saved: {description}**", file=file
    )
    if msg.attachments:
        url = msg.attachments[0].url
...
async def save_server_fact(
    ...
    await consolidate_memories_if_needed(
        client, brain_server_id, "server-lore", str(server.id)
    )
    return True


async def save_global_fact(
    client: discord.Client,
    brain_server_id: int,
    fact: str,
    category: str = "PROFILE & IDENTITY",
    ...
    await consolidate_memories_if_needed(
        client, brain_server_id, "global-memory", "global-database"
    )
    return True
...
<<<<<<< HEAD:core/memory.py
    """
    Forget fact adaptor. Deletes matching factual text blocks from
    both legacy formats and modern vector nodes (by downloading attached JSONs).
    """
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    guild = client.get_guild(brain_server_id)
    ...
    thread_name = (
        re.sub(r"[^a-zA-Z0-9\-]", "", channel_name)
        .replace("-memory", "")
        .replace("-lore", "")
    )
    ...
    async for msg in thread.history(limit=100):
        ...
        if msg.attachments and (
            msg.content.startswith("[FACT_VEC]")
            or any(att.filename.endswith(".json") for att in msg.attachments)
        ):
            try:
        ...
        elif msg.content.startswith("[FACT_VEC]"):
            try:
...
<<<<<<< HEAD:core/memory.py
    """
    Fetches memories with a highly optimized numpy cosine-similarity search.
    Supports file attachment reading while preserving backward compatibility.
    """
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    guild = client.get_guild(brain_server_id)
    ...
    for msg in raw_messages:
        ...
        if msg.attachments and (
            ...
            try:
                ...
                logger.error(f"Failed to read attached vector payload: {e}")

        elif content.startswith("[FACT_VEC]"):
            try:
        ...
        try:
            ...
            for mem in structured_memories:
                ...
                if query_norm == 0.0 or fact_norm == 0.0:
                    ...
                    similarity = float(
                        np.dot(query_vector, fact_vector) / (query_norm * fact_norm)
                    )
            ...
            logger.error(
                f"Semantic similarity retrieval failed: {sim_err}. Falling back to standard list dump..."
            )
...
async def load_config(
    client: discord.Client, brain_server_id: int, target_id: int, is_dm: bool
) -> Optional[dict]:
    guild = client.get_guild(brain_server_id)
    if not guild:
        return None
    ...
    forum = discord.utils.get(
        guild.channels, name="configurations", type=discord.ChannelType.forum
    )
    if not forum:
        return None
...
async def save_context_snippet(
    client,
    brain_server_id: int,
    user_id: int,
    alias: str,
...
async def fetch_all_contexts_for_user(
    client, brain_server_id: int, user_id: int
) -> list:
    guild = client.get_guild(brain_server_id)
    if not guild:
        return []
...
async def save_news_state(
    client: discord.Client, brain_server_id: int, guild_id: int, state_dict: dict
) -> bool:
<<<<<<< HEAD:core/memory.py
    """Saves persistent automated news execution state parameters to the Brain Server configuration forum."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    guild = client.get_guild(brain_server_id)
    if not guild:
...
async def load_news_state(
    client: discord.Client, brain_server_id: int, guild_id: int
) -> Optional[dict]:
<<<<<<< HEAD:core/memory.py
    """Retrieves persistent automated news execution state parameters from the Brain Server configuration forum."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/core/memory.py
    guild = client.get_guild(brain_server_id)
    if not guild:
