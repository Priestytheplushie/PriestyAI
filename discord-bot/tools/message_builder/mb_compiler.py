...
<<<<<<< HEAD:tools/message_builder/mb_compiler.py
    """Raised when the DSL code contains syntax errors or unapproved commands."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/tools/message_builder/mb_compiler.py
...
class LayoutASTVisitor(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        raise ASTValidationError(
            "Imports are strictly forbidden inside layout scripts."
        )
    ...
    def visit_FunctionDef(self, node):
        raise ASTValidationError(
            "Defining functions is forbidden. Write declarative component constructors only."
        )
    ...
    def visit_Call(self, node):
        ...
        if base_func not in self.whitelisted_calls:
            raise ASTValidationError(
                f"Call to unapproved component or function: '{func_name}'"
            )
...
def parse_action_arg(node: ast.AST) -> Union[str, Dict[str, Any], List[Any]]:
    """Recursively processes AST values into safe primitives representing event trigger calls."""
...
<<<<<<< HEAD:tools/message_builder/mb_compiler.py
    """Translates verified Abstract Syntax Trees directly into declarative configurations."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/tools/message_builder/mb_compiler.py
...
def compile_dsl_payload(dsl_code: str) -> Dict[str, Any]:
    """
    Parses and compiles a raw python block, verifying security constraints,
    checking layout rules, and confirming that no modals are chained together.
    """
    ...
    try:
        ...
        raise ASTValidationError(
            f"Syntax Error in Python compilation: {syntax_err.msg} at line {syntax_err.lineno}"
        )
    ...
    if not compiled_layout:
        raise ASTValidationError(
            "Layout compilation failed. No root class assignment or layout instantiation found."
        )
...
def verify_structural_constraints(node: Dict[str, Any]):
    """Recursively validates the component layout mapping against Discord V2 API limitations."""
    ...
    if comp_type == "ActionRow":
        ...
        if dropdowns_count > 1:
            raise ASTValidationError(
                "Discord API Limit: An ActionRow cannot hold more than exactly 1 Select Dropdown menu."
            )
        if dropdowns_count > 0 and buttons_count > 0:
            raise ASTValidationError(
                "Discord API Limit: You cannot mix Buttons and Select Dropdowns in the same ActionRow."
            )
        if buttons_count > 5:
            raise ASTValidationError(
                "Discord API Limit: An ActionRow cannot hold more than 5 Button components."
            )
        if buttons_count == 0 and dropdowns_count == 0:
            raise ASTValidationError(
                "Discord API Limit: An ActionRow must contain between 1 and 5 components."
            )
    ...
    for callback_prop in ("on_click", "on_select", "on_submit"):
        if callback_prop in kwargs:
            ...
            if isinstance(cb_val, list):
                has_modal = any(
                    isinstance(a, dict)
                    and a.get("type", "").split(".")[-1].lower().strip()
                    in ("open_modal", "openmodal", "modal")
                    for a in cb_val
                )
                if has_modal:
                    if len(cb_val) > 1:
