...
def build_layout_generator_prompt(
    initial_prompt: str, ai_banter_reply: str, extracted_instructions: str
) -> str:
<<<<<<< HEAD:tools/message_builder/mb_tool.py
    """
    Constructs a highly structured system-override prompt for the second compile pass.
    Combines initial prompt context, banter details, and parameters.
    """
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/tools/message_builder/mb_tool.py
    return (
...
async def build_message_layout(
    bot_instance,
    channel,
    dsl_script_code: str,
    initial_prompt: str,
    user_app_session_id: Optional[int] = None,
) -> Tuple[Optional[discord.ui.LayoutView], Optional[str]]:
<<<<<<< HEAD:tools/message_builder/mb_tool.py
    """
    Compiles verified AST python blocks into active V2 views.
    Returns the view on success, or translates runtime errors into actionable AI-remedies on failure.
    """
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/tools/message_builder/mb_tool.py
    try:
...
<<<<<<< HEAD:tools/message_builder/mb_tool.py
    """Dynamically registers the thinking turn callbacks on the core bot lifecycle."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/tools/message_builder/mb_tool.py

    async def trigger_message_builder_ai_turn(
        self, interaction: Optional[discord.Interaction], instruction_payload: str
    ):
<<<<<<< HEAD:tools/message_builder/mb_tool.py
        """Processes interaction callbacks, feeding selections back to a thinking context turn."""
=======
>>>>>>> 982e394 (feat: initial monorepo setup for discord bot and github app):discord-bot/tools/message_builder/mb_tool.py
        ...
        async with channel.typing():
            ...
            context = (
                self._compile_server_context(channel.guild, author)
            ...
            if "Memory Journals" in config_state.get("system_tools", []):
                memories = await self._compile_memories_for_ai(
                    author, channel, query_text=instruction_payload
                )
            else:
                memories = {
                    "user_memories": "",
            ...
            base_sys_prompt = (
                custom_prompt if custom_prompt else self.chat_handler.system_instruction
            )
    ...
    bot_class_instance.trigger_message_builder_ai_turn = (
        trigger_message_builder_ai_turn.__get__(bot_class_instance)
    )
