...
async def auto_accept_invitations_worker() -> None:
    """
    Background worker that runs continuously to auto-accept any repository
    collaborator invitations sent to the bot account.
    """
...
def verify_signature(
    ...
    """Verifies HMAC-SHA256 signature from GitHub webhook payload."""
    if not secret:
        return True
...
def extract_resource_key(event_name: str, payload: Dict[str, Any]) -> str:
    """Extracts a granular lock key (e.g. 'owner/repo#42') to sequence operations."""
...
async def dispatch_webhook_event(
    ...
    """Central router for all GitHub events guarded by keyed resource locks."""
