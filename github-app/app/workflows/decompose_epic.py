...
async def handle_decompose_epic(
    ...
    """
    Decomposes an Epic / Mega-Issue into focused child Sub-Issues:
    1. Reads parent issue context & discussion history.
    2. Reasons through architectural phases (2 to 4 sub-issues).
    3. Creates each sub-issue via MachineUserClient (unassigned).
    4. Natively links each child issue to the parent issue via GitHub Sub-Issues API.
    5. Automatically unassigns bot from parent Epic issue to keep container clean.
    6. Leaves a confirmation comment on the parent issue linking to the new sub-issues.
    """
