...
async def handle_discussion_opened(payload: Dict[str, Any]) -> None:
    """
    Triggered on webhook 'discussion.created'.
    - If in Q&A category: provides an initial code-backed teammate answer (top-level comment).
    - If explicitly @mentioned in discussion body: answers directly.
    """
...
async def handle_discussion_comment(payload: Dict[str, Any]) -> None:
    """
    Triggered on webhook 'discussion_comment.created'.
    - Handles comments and replies inside discussion threads.
    - Always posts responses nested inside the comment thread.
    """
...
async def get_discussion_comment_node_id(
    ...
    """Queries GraphQL to get the Node ID of a comment by its database ID."""
    query = """
