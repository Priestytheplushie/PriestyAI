...
async def handle_fix_request(
    ...
    """
    Executes a review-aware FIX_REQUEST:
    1. Validates caller authorization (Maintainer, PR Author, or Bot-authored PR).
    2. Evaluates scope & generates descriptive Conventional Commit title.
    3. Generates targeted patches using Reasoning Tier.
    4. Verifies in Docker with self-healing (skipped for untrusted forks).
    5. Commits atomically with a semantic commit message or provides clean 1-click suggestions.
    6. Re-requests review and posts confirmation.
    """
...
async def verify_and_heal_fix(
    ...
    """Runs Docker checks and performs up to 2 self-healing passes using Reasoning Tier if test errors occur."""
    if (
...
async def detect_repo_stack(file_tree: List[str]) -> Dict[str, Any]:
    """Dynamically determines build/test container environment."""
