...
def clean_branch_slug(title: str) -> str:
    """Strips issue prefixes, scopes, IDs, and special characters into a clean slug."""
...
def sanitize_branch_name(candidate_branch: Optional[str], issue_title: str) -> str:
    """Ensures branch is formatted cleanly with standard prefixes (feature/, fix/, docs/, chore/)."""
...
async def handle_issue_assigned(payload: Dict[str, Any]) -> None:
    """
    Triggered when PriestyAI is assigned to an Issue (or commanded to start):
    1. Checks if issue has existing child sub-tasks (guides user to next open sub-task via LLM).
    2. Checks if a branch was already created under Development sidebar on this issue.
    3. Gathers context & assesses scope (1 to 5 steps).
    4. Opens Draft PR using the linked branch (or clean branch) and assigns maintainer.
    """
