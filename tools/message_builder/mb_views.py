...
class DSLRuntimeView(discord.ui.LayoutView):
    def __init__(self, bot_instance, channel, dsl_view_config: Dict[str, Any], initial_prompt: str, user_app_session_id: Optional[int] = None):
    ...
    async def on_timeout(self) -> None:
        logger.info(f"MessageBuilder LayoutView timed out for channel {self.channel.id}")
...
class DSLButton(discord.ui.Button):
    def __init__(self, label: str, style: discord.ButtonStyle, url: Optional[str], on_click_actions: Any, custom_id: str, bot_instance, channel):
        super().__init__(label=label, style=style, url=url, custom_id=custom_id)
        self.actions = on_click_actions if isinstance(on_click_actions, list) else [on_click_actions]
        self.bot = bot_instance
        self.channel = channel
...
class DSLUserSelect(discord.ui.UserSelect):
    def __init__(self, placeholder: str, min_values: int, max_values: int, on_select_actions: Any, custom_id: str, bot_instance, channel):
        super().__init__(placeholder=placeholder, min_values=min_values, max_values=max_values, custom_id=custom_id)
        self.actions = on_select_actions if isinstance(on_select_actions, list) else [on_select_actions]
        self.bot = bot_instance
        self.channel = channel
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await process_action_chain(self.bot, interaction, self.channel, self.actions, summary)


class DSLStringSelect(discord.ui.Select):
    def __init__(self, placeholder: str, options: List[discord.SelectOption], min_values: int, max_values: int, on_select_actions: Any, custom_id: str, bot_instance, channel):
        super().__init__(placeholder=placeholder, options=options, min_values=min_values, max_values=max_values, custom_id=custom_id)
        self.actions = on_select_actions if isinstance(on_select_actions, list) else [on_select_actions]
        self.bot = bot_instance
        self.channel = channel
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await process_action_chain(self.bot, interaction, self.channel, self.actions, selected_value)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Label):
                ...
                if isinstance(nested, discord.ui.TextInput):
                ...
                elif isinstance(nested, (discord.ui.CheckboxGroup, discord.ui.RadioGroup)):
                    selected_vals = nested.values
                    results.append(f"{label_text}: {', '.join(selected_vals) if selected_vals else 'None'}")
                    ...
                    results.append(f"{label_text}: [Uploaded Files: {', '.join(file_names) if file_names else 'None'}]")
                elif isinstance(nested, (discord.ui.Select, discord.ui.UserSelect, discord.ui.RoleSelect, discord.ui.ChannelSelect, discord.ui.MentionableSelect)):
                    selected_vals = []
        ...
        await process_action_chain(self.bot, interaction, self.channel, self.actions, summary_payload)
...
def build_layout_item(cfg: Dict[str, Any], bot_instance, channel) -> Optional[Any]:
    ...
    if comp_type == "Container":
        ...
        container = discord.ui.Container(accent_colour=accent_col) if accent_col else discord.ui.Container()
        ...
        spacing_enum = discord.SeparatorSpacing.large if spacing_val == "large" else discord.SeparatorSpacing.small
...
def build_layout_tree(parent_view: discord.ui.LayoutView, children_configs: List[Dict[str, Any]]):
...
async def process_action_chain(bot_instance, interaction: discord.Interaction, channel, actions_list: List[Any], context_value: Optional[str] = None):
    for action in actions_list:
        ...
        if action_type == "disable_components":
            ...
            reply_text = action_args[0] if action_args else action_kwargs.get("text_content", "Acknowledge selection.")
            if context_value:
            ...
            if interaction.response.is_done():
                ...
                await interaction.response.send_message(content=reply_text, ephemeral=True)

        elif action_type == "reply_public":
            reply_text = action_args[0] if action_args else action_kwargs.get("text_content", "")
            if context_value:
                reply_text = reply_text.replace("{value}", context_value)
            ...
            if interaction.response.is_done():
                await interaction.followup.send(content=pass_confirmation, ephemeral=True)
            else:
                await interaction.response.send_message(content=pass_confirmation, ephemeral=True)

        elif action_type == "watch_channel":
            channel_id = int(context_value) if (context_value and context_value.isdigit()) else channel.id
            ...
            img_prompt = action_args[0] if action_args else action_kwargs.get("prompt", "")
            if context_value:
            ...
            bot_instance.loop.create_task(
                bot_instance._generate_decoupled_image(
                    channel=channel, author=interaction.user, raw_image_prompt=img_prompt,
                    placeholder_msg=placeholder, context_history=bot_instance.history_tracker.get_formatted_history(channel.id),
                    is_edit_flow=False, original_message=None, banter="",
                    disabled_triggers={}, config=active_config
                )
            )
            ...
            m_title = m_args[0] if len(m_args) > 0 else m_kwargs.get("title", "Form Popup")
            
            if interaction.response.is_done():
                await interaction.followup.send(
                    "❌ Discord API Constraint Error: Modals cannot be launched after "
                    "another interaction response has been triggered.", ephemeral=True
                )
                return
                
            modal_obj = DSLV2Modal(
                title=m_title, on_submit_actions=m_kwargs.get("on_submit"), 
                bot_instance=bot_instance, channel=channel
            )
            ...
            for field in fields:
                ...
                if field_type == "Label":
                    ...
                    if sub_comp_cfg and isinstance(sub_comp_cfg, dict):
                        ...
                        custom_field_id = sc_kwargs.get("id") or f"fld_{uuid.uuid4().hex[:8]}"
                        ...
                        if sc_type == "CheckboxGroup":
                            ...
                            sub_comp = discord.ui.ChannelSelect(custom_id=custom_field_id)
                        elif sc_type == "MentionableSelect":
                            sub_comp = discord.ui.MentionableSelect(custom_id=custom_field_id)
            ...
            instruction_payload = action_args[0] if action_args else action_kwargs.get("instruction_payload", "")
            if context_value:
                instruction_payload = f"{instruction_payload} | Value Selected: {context_value}"
                
            if not interaction.response.is_done():
                await interaction.response.defer()
                
            await bot_instance.trigger_message_builder_ai_turn(interaction, instruction_payload)
